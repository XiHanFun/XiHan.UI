...
<template>
  <div style="display: flex; align-items: center; gap: 8px">
    <!-- 不写 platform 即挂载后自己测；命中默认拦下浏览器的默认动作 -->
...
<div style="display: flex; align-items: center; gap: 8px">
  <!-- 不写 platform 即挂载后自己测；命中默认拦下浏览器的默认动作 -->
  <xh-hotkeys id="hotkeys-basic" keys="Mod,S">
...
`background` · `box-shadow` · `color` 走 `transition` 过渡。时长与缓动读[动效令牌](../guide/motion)，改令牌即改全局节奏。
