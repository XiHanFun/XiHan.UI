const t=`<!-- 外部写值与清空 | 值由宿主持有，按钮直接写值或清空；填齐与越界两个判据由组件给出，按钮照它们摆 -->
<xh-date-field id="date-field-actions" value="" locale="zh-CN">
  <div data-xh-part="root">
    ...
    <div style="display: flex; gap: 8px">
      ...
      <!-- 一段都没填时这颗按钮按不动 -->
      <xh-button id="date-field-actions-clear" size="sm" variant="ghost" disabled>
...
<script type="module">
  ...
  const clear = document.getElementById("date-field-actions-clear");
  ...
  // 空、填齐与越界三个判据落在根节点上，等这一轮更新写完再读
  async function sync() {
    await field.updateComplete;
    clear.toggleAttribute("disabled", root.hasAttribute("data-empty"));
