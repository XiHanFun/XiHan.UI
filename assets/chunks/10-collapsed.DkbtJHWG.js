...
export default function Demo(): ReactNode {
  ...
  return (
    <div style={{ inlineSize: "100%", display: "flex", gap: "16px", paddingBlockEnd: "40px" }}>
      <XhNavigationMenuRoot
          ...
          {groups.map(g => (
            <XhNavigationMenuItem key={g.value}>
              ...
              >
                ...
                {collapsed ? null : <span>{g.label}</span>}
              </XhNavigationMenuTrigger>
              <XhNavigationMenuContent value={g.value}>
                {collapsed
                  ? (
                      <span style={{ padding: "2px 8px", color: "var(--xh-fg-muted)", fontSize: "12px" }}>
                        {g.label}
                      </span>
                    )
                  : null}
      ...
      <XhButton size="sm" variant="outline" onClick={() => setCollapsed(!collapsed)}>
        {collapsed ? "展开侧栏" : "收窄侧栏"}
      </XhButton>
    </div>
  );
