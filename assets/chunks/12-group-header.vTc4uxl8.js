const e=`// 多行表头与表头分组 | 表头写几行就是几行；分组格的跨列数与两行表头的行号由标记自报，columns 仍只登记叶子列
...
const groupStyle: CSSProperties = { inlineSize: "10rem", flexGrow: 2 };
    ...
    <div style={{ width: "100%", maxWidth: "620px" }}>
      ...
      <XhTableRoot columns={columns} rows={rows} aria-rowcount={teams.length + 2}>
        ...
        <XhTableBody>
          {/* 数据行也往后挪一行 */}
          {teams.map((t, i) => (
            <XhTableRow key={t.id} value={t.id} aria-rowindex={i + 3}>
