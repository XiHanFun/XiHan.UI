...
const recent = {
  key: "recent",
  label: "最近一年",
  start: formatHeatmapDate(today - 364 * DAY_MS),
  end: formatHeatmapDate(today),
};
...
export default function Demo(): ReactNode {
  const [activeKey, setActiveKey] = useState(recent.key);
  const active = ranges.find(r => r.key === activeKey) ?? recent;
  // 数据跟着区间换：区间外的日子不进网格，也不把档位标尺顶高
  const activity = useMemo(() => buildRange(active.start, active.end), [active.start, active.end]);
