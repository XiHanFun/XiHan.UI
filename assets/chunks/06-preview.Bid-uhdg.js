const n=`// 点击查看大图 | 缩略图的点击与键盘自行接管，放大层是一个对话框，其中再放一份独立的图片实例
...
export default function Demo(): ReactNode {
  ...
  function zoom(step: number): void {
    setScale(prev => Math.min(3, Math.max(0.5, prev + step)));
  }
    ...
    <>
      ...
      >
        <XhDialogContent>
          ...
          <XhImageRoot src={photo} alt="山间日出，放大查看" style={largeStyle}>
            <XhImageImage
              style={{
          ...
          <XhToolbarRoot>
            <XhToolbarItem value="zoom-in" style={itemStyle} onClick={() => zoom(0.25)}>
              放大
            </XhToolbarItem>
            <XhToolbarItem value="zoom-out" style={itemStyle} onClick={() => zoom(-0.25)}>
              缩小
            </XhToolbarItem>
            <XhToolbarItem value="rotate" style={itemStyle} onClick={() => setRotate(prev => prev + 90)}>
              旋转
            </XhToolbarItem>
            <XhToolbarItem
              value="reset"
