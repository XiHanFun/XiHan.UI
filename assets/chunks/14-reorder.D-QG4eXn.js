const e=`// 拖拽换位 | 整个标签都是拖动源：按住往旁边拖，落点画成一条线、被拖的标签原地不动；也可以聚焦标签带后按 Alt + 左右键挪一位（竖排是 Alt + 上下键），到首末就不动。库不拥有标签序，只报一次重排好的新顺序连同读屏播报，照它写回数组归使用者
...
export default function Demo(): ReactNode {
  ...
  const [active, setActive] = useState<string | null>("outline");
  const [log, setLog] = useState("按住标签拖到别处，或聚焦标签带后按 Alt + 左右键");
  ...
  function onTabMove(move: TabMove): void {
    ...
    setTabs(move.values.flatMap(value => byValue.get(value) ?? []));
    setLog(\`\${byValue.get(move.value)?.label ?? move.value} 挪到了第 \${move.to + 1} 位\`);
  }
