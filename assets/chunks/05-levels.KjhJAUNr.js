const e=`// 级别 | 行上写 level，四档 debug / info / warn / error 由皮肤染色；时间戳与行内标记仍归作者
...
export default function Demo(): ReactNode {
  return (
    <XhLogRoot rows={6} style={{ inlineSize: "100%" }}>
      <XhLogViewport>
        <XhLogContent>
          {entries.map((entry, i) => (
            <XhLogLine key={i} level={entry.level}>
