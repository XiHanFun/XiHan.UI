...
<xh-dynamic-input id="dynamic-input-object" movable>
  <div data-xh-part="root" style="max-inline-size: 480px">
    <button data-xh-part="add-trigger">+ 添加请求头</button>
  </div>
</xh-dynamic-input>

<pre id="dynamic-input-object-dump"></pre>

<template id="dynamic-input-object-row">
...
<script type="module">
  const host = document.getElementById("dynamic-input-object");
  const root = host.querySelector('[data-xh-part="root"]');
  const addTrigger = host.querySelector('[data-xh-part="add-trigger"]');
  const template = document.getElementById("dynamic-input-object-row");
  const dump = document.getElementById("dynamic-input-object-dump");
