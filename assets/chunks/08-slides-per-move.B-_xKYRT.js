...
<script type="module">
  // 当前页写在指示点的 data-selected 上，露在外面的那几张带 data-inview，回显直接读它们
  ...
  function render() {
    const page = indicators.findIndex((el) => el.hasAttribute("data-selected"));
  ...
  new MutationObserver(render).observe(root, {
    attributes: true,
    subtree: true,
    attributeFilter: ["data-selected", "data-inview"],
  });
