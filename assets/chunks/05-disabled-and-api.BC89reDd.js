const t=`<!-- 禁用与程序化操作 | 禁用时三类把手全按不动；从外面加一条走同一条闸门，整份替换值则不受闸门约束 -->
<label>
  <input id="dynamic-input-lock" type="checkbox" />
  锁定这份清单
</label>

<xh-dynamic-input id="dynamic-input-api">
  <div data-xh-part="root" style="max-inline-size: 420px">
    <button data-xh-part="add-trigger">+ 添加任务</button>
    <p>
      <button type="button" id="dynamic-input-api-add">从外面加一条</button>
      <button type="button" id="dynamic-input-api-restore">恢复默认</button>
    </p>
  </div>
</xh-dynamic-input>

<template id="dynamic-input-api-row">
...
<script type="module">
  const host = document.getElementById("dynamic-input-api");
  const root = host.querySelector('[data-xh-part="root"]');
  const addTrigger = host.querySelector('[data-xh-part="add-trigger"]');
  const template = document.getElementById("dynamic-input-api-row");
  ...
  document.getElementById("dynamic-input-lock").addEventListener("change", (event) => {
    host.disabled = event.target.checked;
  });

  // 转交给已接线的新增把手，锁定时同样按不动
  document.getElementById("dynamic-input-api-add").addEventListener("click", () => {
    addTrigger.click();
  });

  // 整份替换值不经把手，锁定时照样生效
  document.getElementById("dynamic-input-api-restore").addEventListener("click", () => {
    tasks = [...DEFAULTS];
    host.value = tasks;
    render();
  });
