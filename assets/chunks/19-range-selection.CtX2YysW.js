const e=`// 范围选择 | 按住 Shift 点击勾选框选中一段；焦点落在表体中按 Ctrl/Cmd + A 全选。禁用行占据顺序位置但不被选入
  ...
  return (
    <div style={{ width: "100%", maxWidth: "560px", display: "grid", gap: "12px" }}>
      <p style={{ color: "var(--xh-fg-muted)" }}>
        ...
        {" 点第五行 —— 中间整段一起选上（禁用那行跳过）。再按住 Shift 点第三行，选区会往回收，起点不变。焦点落在表体里按 "}
        <kbd>Ctrl</kbd>
        /
        <kbd>Cmd</kbd>
        {" + "}
      ...
      >
        ...
        <XhTableBody>
          {files.map(f => (
            <XhTableRow key={f.id} value={f.id}>
