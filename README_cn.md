...
- **135 个组件** - 覆盖通用、布局、导航、数据录入、数据展示、反馈、浮层、AI 对话八组
...
改动需通过 CI 全套门禁，CI 与本地同一套命令：`pnpm lint`、`pnpm typecheck`、`pnpm boundaries`、`pnpm gate`（一条命令跑 128 项结构检查）、`pnpm test`、`pnpm build`、`pnpm size` 等。

## 覆盖范围

库里有的：135 个组件的内核与三个适配器、127 个视觉组件的默认皮肤、设计令牌与主题运行时、跨适配器一致性套件、真实 Chromium 里的无障碍扫描与浮层定位契约、文档站。
