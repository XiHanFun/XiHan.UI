来源：https://ui.docs.xihanfun.com/runtime/services

# 命令式服务

有些东西写成模板反而绕：删除前问一句、保存完提示一下——这些没有"挂在哪"的问题，就该一次调用弹出来。库提供四个服务工厂：对话框、轻提示、通知与顶部进度条。

四者都自己建宿主容器、自己管挂载与卸载，**用完记得 `dispose()`**。
...
后来的排队顺次弹出，避开多层模态叠加。当前项的内容与遮罩全部完成有限退场动画后，队列才放出下一项；无动画或减动效时不额外等待固定时间。
