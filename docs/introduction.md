...
当前提供 **123 个组件**，每个组件同时产出四份东西——无头内核、Vue 组件、自定义元素、默认皮肤。四份同源，行为由内核唯一定义，适配器不重新实现任何逻辑。
...
```
   ...
   无头内核          │       @xihan-ui/headless        │   ← 解剖 + 状态机 + connect
                    └────────────────┬────────────────┘
              ┌────────────────────┬─┴────────────┬─────────┐
   原语层      │ @xihan-ui/core     │ position     │ motion  │
              └────────────────────┴──────────────┴─────────┘
                    ┌────────────────────────────────┐
...
- **无障碍是判据，不是口号。** 每个组件都有一份机读的**键盘规格表**（共 498 条），它同时是测试的分母：用例少覆盖一条即判套件失败。
