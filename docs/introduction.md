...
当前提供 **128 个组件**，每个组件都有无头内核、Vue 组件、React 组件与自定义元素；127 个视觉组件另有默认皮肤，renderless Hotkeys 不伪造视觉层。各层同源，行为由内核唯一定义，三个适配器负责接入各自的渲染与事件机制。
...
- **无障碍是判据，不是口号。** 每个组件都有一份机读的**键盘规格表**（共 534 条），它同时是测试的分母：用例少覆盖一条即判套件失败。
