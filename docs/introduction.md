...
::: warning 实验阶段
XiHan.UI 尚未发布到 npm（库包版本仍是 `0.0.0`），也还没有可交互的组件预览站。无障碍扫描已经跑在真实 Chromium 上，首轮扫出的存量问题已经清完，登记表里只剩两条（WC 侧 steps 的必需子节点、breadcrumb 的步骤重放豁免）。**请勿在生产环境依赖。**
:::
...
```
   ...
   原语层      │ core         │ machine       │ behavior     │ position
              └──────────────┴───────────────┴──────────────┘
                    ┌────────────────────────────────┐
   表现层            │ @xihan-ui/tokens  ·  styled     │   ← 令牌与皮肤，纯 CSS，与 JS 无关
                    └────────────────────────────────┘
```
...
- **无障碍是判据，不是口号。** 每个组件都有一份机读的**键盘规格表**（共 375 条），它同时是测试的分母：用例少覆盖一条即判套件失败。
