...
现在的 XiHan.UI 是 2026-07-25 起从零重写的框架无关设计系统运行时，15 个公开包（`@xihan-ui/*`）由 changesets 的 `fixed` 组锁步同版，当前版本 `1.0.0-alpha.1`，**已发布到 npm**（`latest` 与 `alpha` 两个 dist-tag 都指向它）。它是预发布：不承诺语义化版本，接口还会变，不要用于生产。
