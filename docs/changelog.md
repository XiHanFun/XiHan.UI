...
`1.0.0` 的预览版，走 `preview` dist-tag。公开面已按[版本与兼容性政策](/guide/versioning)逐类整理完毕，该页条款自 `1.0.0` 正式版起生效。

这一节写的是**整套库有什么**，不是与上一版的差集；逐版的改动仍在下面的 alpha 四节里。
...
### 自 1.0.0-alpha.3 以来（45 份变更集）

这一段的主线是**把 alpha 里遗留的形状问题一次改完**：两对做着同一件事的组件分家（toast / notification、badge / tag），三处与同族对不上的 prop 名归位，语气色从逐族写死改为按底色现推。
