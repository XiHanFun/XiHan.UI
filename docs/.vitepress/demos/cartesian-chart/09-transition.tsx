...
export default function Demo(): ReactNode {
  ...
  const data = months.map((month, i) => ({ month, online: current.online[i], store: current.store[i] }));
  ...
  return (
    <div style={{ display: "grid", gap: "var(--xh-space-4)", width: "100%" }}>
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "var(--xh-space-4)" }}>
        <XhButton onClick={() => setIndex(i => (i + 1) % years.length)}>换一组数据</XhButton>
      ...
      <XhCartesianChartRoot
        ...
        series={series}
        animated={animated}
        caption={`${current.year} 年上半年销售额（万元）`}
      />
    </div>
  );
}
