...
export default function Demo(): ReactNode {
  ...
  return (
    <XhSortableRoot ids={ids} onSort={({ ids: next }) => setIds(next)} style={{ inlineSize: "min(360px, 100%)" }}>
      {ids.map((id, index) => (
        <XhSortableItem key={id} itemId={id} aria-label={id} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 12px", borderRadius: "var(--xh-shape-surface)", background: "var(--xh-bg-subtle)" }}>
