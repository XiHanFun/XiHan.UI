// 控件在薄封装里 | 封装的根不是可聚焦元素时，关掉 asChild、让封装内部用 useFieldControl 自取
import type { ReactNode } from "react";
import { useFieldControl, XhFieldControl, XhFieldDescription, XhFieldLabel, XhFieldRoot } from "@xihan-ui/react";

// 典型的薄封装：根是个 div，真正可聚焦的 input 在里面
function MyInput(): ReactNode {
  const controlProps = useFieldControl();
  return (
    <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
      <span>@</span>
      <input {...controlProps} placeholder="you@example.com" style={{ flex: 1 }} />
    </div>
  );
}
