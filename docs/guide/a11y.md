...
每一行有稳定的 `id`、触发按键、生效前置条件、以及效果。全库共 **514 条**，散落在 130 个组件上。
...
当前共三条。两个适配器共用的两条：`tag` 的禁用标签文字用的是全库统一的 `fg-disabled` / `bg-muted`（2.36:1），按 1.4.3 的失效控件豁免不算违规，但 tag 的 root 没有 axe 认得的禁用语义，所以只有它被扫出来；`prompt-input` 的输入框刻意不无条件发 `aria-label`（那会盖掉作者的 `<label for>`），一致性夹具刻意不给文案以钉住这条契约，扫出来就是一个没有名字的 textarea，真实用法里由作者命名。Web Components 适配器自己的一条：`steps` 的必需子节点由作者手写，可能缺角色要求的直接子节点。全局登记为空。
