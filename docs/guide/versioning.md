...
| observed attribute | 996 条声明 / 308 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，9376 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 184 条子入口、4304 个导出名、119 个 `data-scope` 与 747 条部件配对、
119 个组件的 1272 个 prop 名、156 种 `data-*`、20 个 `data-state` 取值、294 个令牌、
