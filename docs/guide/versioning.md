...
| Vue 组件导出 `Xh*` | 1004（132 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
| Vue 组合式函数 `use<家族>` | 101 | `useSelect`、`useCombobox`。这是「不用我的部件、自己写标记」的唯一入口 |
| Vue 指令 | 2 | `vBackground`（`@xihan-ui/vue/backgrounds`）、`vSound`（`@xihan-ui/vue/sound`），两个子入口各依赖一个可选 peer |
| 无头内核 `connect*` | 132 | `connectAccordion` 及其参数顺序、返回的 getter 名 |
...
| `custom-elements.json`（CEM） | 1 份 / 134 个元素 | 已经进过清单的 `tagName` / `attribute` / `event` 条目不会消失；`cssProperties`（皮肤覆盖槽）与 `events` 的 `type`（detail 类型）由 `scripts/enrich-cem.mjs` 从皮肤与元素源码生成，`gate:cem` 校验同步。字段结构细节仍不承诺，补充算 minor |
...
| `xxxAnatomy` / `xxxMeta` / `xxxKeyboard` 三组导出对象 | 各 132 | **它们描述的 part 名单是受约束的（见第二节），但这三个对象本身的组织方式不是。** 想拿到部件名单，请以组件文档页的解剖表为准，不要 import 这些对象 |
...
| WC 的元素类导出 `Xh*Element` | 106 | 只作 `instanceof` 与手动 `customElements.define` 的便利品，**不支持 `extends`**（基类不导出、`wire()` 是 protected abstract）。要拿元素请用 `document.querySelector` |
| WC 元素上的 `static partContract` | 132 | 部件校验的输入数据，实现细节 |
...
| 类别 | 数量 | 档位 |
| --- | --- | --- |
| `data-scope` 取值（组件身份） | 132 | **受约束**（新增第 133 个组件是 minor） |
| `data-part` 取值（部件名） | 261 个不同名字 / 1003 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 261 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 290 条 | **受约束**（加条目 = major），方向见下 |
...
`connect` 一共产出 221 个不同的 `data-*` 属性名、1020 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 166 个属性名 / 763 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3879（覆盖 131 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| `@xihan-ui/styles` 的 CSS 子路径 | 152 | `.`、`./index.css`、`./index.unlayered.css`，与 143 条 `.css`——132 份组件皮肤加 `./layers.css`、`./tone.css`、`./reset.css`、`./overlay-arrow.css`、`./visually-hidden.css`、`./undefined.css`、`./focus.css`、`./label.css`、`./description.css`、`./pointer.css`、`./forced-colors.css` |
...
| 类别 | 数量 | 档位 |
| --- | --- | --- |
| 自定义元素标签 `xh-*` | 135（`defineXhElements()` 注册 134 + `xh-background`） | **受约束** |
| 注册函数 | 2（`defineXhElements`、`defineXhBackground`） | **受约束** |
| observed attribute | 1315 条声明 / 375 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 375 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
| `CustomEvent` 名 | 90 个名字 / 200 条「元素 × 事件」 | **受约束** |
| 事件传播语义 | `bubbles: true, composed: true`（185 处中 183 处） | **受约束**——把冒泡改掉会让祖先节点上的事件委托静默失效。例外是名为 `submit` 的事件（`xh-prompt-input` / `xh-question-flow`）：与原生表单提交同名，一律不冒泡，免得被祖先 `<form>` 当成自己的提交 |
| 事件 `detail` 形状 | 183 个 `*Details` 类型 | **受约束**，等同于 headless 的同名类型 |
| `attribute: false` 的 JS 字段 | 207 条（涉及 68 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 这类只能用 JS 赋值，HTML 里表达不出来——**不是每个 property 都有对应 attribute** |
| 命令式方法 | 89（分布在 46 个元素） | **受约束**，含参数与返回类型 |
...
- 元素在 `@xihan-ui/web-components/define`，不在包主入口。`defineXhElements()` 是全有全无——调它就注册全部 134 个元素，没有逐个的 `defineXhButton()`。（补细粒度 define 是 minor；补完之后 `defineXhElements` 的「全量」语义就成了承诺。）
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 1004 个组件、101 个组合式函数 |
| `@xihan-ui/react` | 组件与 hooks（铺开中，公开面随批次增长） |
| `@xihan-ui/web-components` | 135 个自定义元素 |
| `@xihan-ui/headless` | `connect*` / `*Machine` / 各类公开类型；内部算子在排除清单里 |
| `@xihan-ui/styles` | 132 份组件皮肤、5 个层名 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，15051 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 188 条子入口、7833 个导出名、132 个 `data-scope` 与 1003 条部件配对、
132 个组件的 1695 个 prop 名、223 种 `data-*`、34 个 `data-state` 取值、418 个令牌、
5 个 `@layer` 名、3879 个组件覆盖槽、134 个自定义元素及其 attribute 与事件。
...
写错值编译期就报错。**Vue 事件载荷也有类型了**，97 个组件的 `emits` 全是对象式，
