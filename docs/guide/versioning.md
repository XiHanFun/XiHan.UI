...
`connect` 一共产出 221 个不同的 `data-*` 属性名、1031 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 166 个属性名 / 767 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| observed attribute | 1324 条声明 / 374 个不同名字 | **受约束**（具体元素上的具体属性名） |
