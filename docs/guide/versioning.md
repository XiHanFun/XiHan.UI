...
| 组件覆盖槽 | 3635（覆盖 126 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| observed attribute | 1236 条声明 / 373 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
覆盖：包名与 182 条子入口、7443 个导出名、128 个 `data-scope` 与 944 条部件配对、
128 个组件的 1591 个 prop 名、221 种 `data-*`、34 个 `data-state` 取值、416 个令牌、
5 个 `@layer` 名、3635 个组件覆盖槽、130 个自定义元素及其 attribute 与事件。
