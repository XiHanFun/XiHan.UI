...
| Vue 组件导出 `Xh*` | 1039（134 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| `data-part` 取值（部件名） | 262 个不同名字 / 1041 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 262 个 | **受约束** |
...
| 组件覆盖槽 | 4042（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，在 `:root` 中设置即可修改该组件 |
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 1039 个组件、102 个组合式函数 |
...
以入库基线（`ui/tooling/public-surface.json`，16052 个名字）比对当前状态：
基线中有而当前没有，即为删除或改名，构建失败。新增一律放行，因为新增是 minor。

覆盖：包名与 194 条子入口、8359 个导出名、134 个 `data-scope` 与 1041 条部件配对、
134 个组件的 1720 个 prop 名、232 种 `data-*`、33 个 `data-state` 取值、658 个令牌、
5 个 `@layer` 名、4042 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
