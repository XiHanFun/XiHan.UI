...
| Vue 组件导出 `Xh*` | 972（128 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
| Vue 组合式函数 `use<家族>` | 98 | `useSelect`、`useCombobox`。这是「不用我的部件、自己写标记」的唯一入口 |
...
| `custom-elements.json`（CEM） | 1 份 / 130 个元素 | 已经进过清单的 `tagName` / `attribute` / `event` 条目不会消失；`cssProperties`（皮肤覆盖槽）与 `events` 的 `type`（detail 类型）由 `scripts/enrich-cem.mjs` 从皮肤与元素源码生成，`gate:cem` 校验同步。字段结构细节仍不承诺，补充算 minor |
...
| `@xihan-ui/headless` 的内部伴生类型 | 564 | `*Refs`（机器持有的 DOM 引用袋，44 个）、`ColorPickerHsva`、`CascaderLevel` 等，是上面那批函数的参数与返回类型 |
| `xxxAnatomy` / `xxxMeta` / `xxxKeyboard` 三组导出对象 | 各 128 | **它们描述的 part 名单是受约束的（见第二节），但这三个对象本身的组织方式不是。** 想拿到部件名单，请以组件文档页的解剖表为准，不要 import 这些对象 |
| Vue 的 `provide*` / `use*Context` 函数 | 109 | [Vue 适配器](../adapters/vue) 早已写明「父子组件之间的 provide / inject 是内部实现，不对外开放」。要下探请用 `use<家族>()` |
...
| WC 的元素类导出 `Xh*Element` | 102 | 只作 `instanceof` 与手动 `customElements.define` 的便利品，**不支持 `extends`**（基类不导出、`wire()` 是 protected abstract）。要拿元素请用 `document.querySelector` |
| WC 元素上的 `static partContract` | 128 | 部件校验的输入数据，实现细节 |
...
| `data-part` 取值（部件名） | 259 个不同名字 / 971 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 259 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 277 条 | **受约束**（加条目 = major），方向见下 |
...
其余 `data-xh-*` 属性（`data-xh-scrollbar`、`data-xh-focus-guard`、`data-xh-inert-exempt` 等 32 个）是库自用标记，**排除**在承诺之外。但 `data-xh-part` 不是——它是 Web Components 适配器唯一的作者输入 API：你写 `data-xh-part="trigger"` 是**声明**，元素接线后往同一节点打上 `data-scope` + `data-part` 是**事实**。皮肤匹配后者，你永远不该手写后者。
:::
...
`connect` 一共产出 219 个不同的 `data-*` 属性名、970 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 166 个属性名 / 740 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3724（覆盖 127 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| `@xihan-ui/styles` 的 CSS 子路径 | 147 | `.`、`./index.css`、`./index.unlayered.css`，与 139 条 `.css`——128 份组件皮肤加 `./layers.css`、`./tone.css`、`./reset.css`、`./overlay-arrow.css`、`./visually-hidden.css`、`./undefined.css`、`./focus.css`、`./label.css`、`./description.css`、`./pointer.css`、`./forced-colors.css` |
...
| observed attribute | 1266 条声明 / 374 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 374 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
| `CustomEvent` 名 | 90 个名字 / 195 条「元素 × 事件」 | **受约束** |
| 事件传播语义 | `bubbles: true, composed: true`（180 处中 178 处） | **受约束**——把冒泡改掉会让祖先节点上的事件委托静默失效。例外是名为 `submit` 的事件（`xh-prompt-input` / `xh-question-flow`）：与原生表单提交同名，一律不冒泡，免得被祖先 `<form>` 当成自己的提交 |
| 事件 `detail` 形状 | 179 个 `*Details` 类型 | **受约束**，等同于 headless 的同名类型 |
| `attribute: false` 的 JS 字段 | 200 条（涉及 68 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 这类只能用 JS 赋值，HTML 里表达不出来——**不是每个 property 都有对应 attribute** |
| 命令式方法 | 83（分布在 43 个元素） | **受约束**，含参数与返回类型 |
...
- 元素在 `@xihan-ui/web-components/define`，不在包主入口。`defineXhElements()` 是全有全无——调它就注册全部 130 个元素，没有逐个的 `defineXhButton()`。（补细粒度 define 是 minor；补完之后 `defineXhElements` 的「全量」语义就成了承诺。）
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 972 个组件、98 个组合式函数 |
...
| `@xihan-ui/styles` | 129 份组件皮肤、5 个层名 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，14587 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 183 条子入口、7621 个导出名、128 个 `data-scope` 与 971 条部件配对、
128 个组件的 1634 个 prop 名、221 种 `data-*`、34 个 `data-state` 取值、416 个令牌、
5 个 `@layer` 名、3724 个组件覆盖槽、130 个自定义元素及其 attribute 与事件。
...
写错值编译期就报错。**Vue 事件载荷也有类型了**，94 个组件的 `emits` 全是对象式，
