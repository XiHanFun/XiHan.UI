...
| 组件覆盖槽 | 3423（覆盖 125 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
5 个 `@layer` 名、3423 个组件覆盖槽、128 个自定义元素及其 attribute 与事件。
