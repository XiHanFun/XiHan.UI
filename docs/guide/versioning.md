...
`connect` 一共产出 225 个不同的 `data-*` 属性名、1481 条「组件 × 属性」配对。分两类。
...
| 组件覆盖槽 | 4014（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，在 `:root` 中设置即可修改该组件 |
...
以入库基线（`ui/tooling/public-surface.json`，15293 个名字）比对当前状态：
...
5 个 `@layer` 名、4014 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
