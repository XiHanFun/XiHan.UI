...
| Vue 组件导出 `Xh*` | 926（126 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| `data-part` 取值（部件名） | 259 个不同名字 / 943 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 259 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 271 条 | **受约束**（加条目 = major），方向见下 |
...
`connect` 一共产出 219 个不同的 `data-*` 属性名、947 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 165 个属性名 / 725 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3622（覆盖 125 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| `@xihan-ui/styles` 的 CSS 子路径 | 145 | `.`、`./index.css`、`./index.unlayered.css`，与 137 条 `.css`——126 份组件皮肤加 `./layers.css`、`./tone.css`、`./reset.css`、`./overlay-arrow.css`、`./visually-hidden.css`、`./undefined.css`、`./focus.css`、`./label.css`、`./description.css`、`./pointer.css`、`./forced-colors.css` |
...
| observed attribute | 1226 条声明 / 374 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 926 个组件、96 个组合式函数 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，14205 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 181 条子入口、7424 个导出名、126 个 `data-scope` 与 943 条部件配对、
126 个组件的 1579 个 prop 名、221 种 `data-*`、34 个 `data-state` 取值、416 个令牌、
5 个 `@layer` 名、3622 个组件覆盖槽、128 个自定义元素及其 attribute 与事件。
