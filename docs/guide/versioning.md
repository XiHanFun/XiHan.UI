...
`connect` 一共产出 162 个不同的 `data-*` 属性名、796 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 126 个属性名 / 610 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3058（覆盖 124 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
::: warning 命名前缀不能反推归属
`--xh-field-py` 长得像 `field` 组件的覆盖槽，实际是全局语义令牌，而且 `field.css` 自己都不用它。同理 `--xh-text-*`（13 个全局文本令牌）与 `text-field` 的 48 条组件槽同前缀，`--xh-color-*`（41 个原语调色板令牌）与 `color-picker` 的 70 条组件槽同前缀。判断一条属性属于哪一档，看它在不在上表列的那 328 个全局令牌里，不要按前缀猜。
:::
...
| observed attribute | 1088 条声明 / 344 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
| `@xihan-ui/tokens` | 328 个令牌名，外加 `./runtime` 的主题控制器与种子色 API |
| `@xihan-ui/icons` | 图标集 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，10505 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 169 条子入口、4704 个导出名、127 个 `data-scope` 与 845 条部件配对、
127 个组件的 1406 个 prop 名、164 种 `data-*`、28 个 `data-state` 取值、328 个令牌、
5 个 `@layer` 名、3058 个组件覆盖槽、129 个自定义元素及其 attribute 与事件。
