...
**样式钩子**——自带皮肤自己就消费了 165 个属性名 / 770 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3891（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| observed attribute | 1338 条声明 / 379 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，15158 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 190 条子入口、7896 个导出名、134 个 `data-scope` 与 1008 条部件配对、
134 个组件的 1722 个 prop 名、222 种 `data-*`、33 个 `data-state` 取值、419 个令牌、
5 个 `@layer` 名、3891 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
