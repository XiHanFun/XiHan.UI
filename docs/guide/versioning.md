...
**样式钩子**——自带皮肤自己就消费了 166 个属性名 / 733 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| observed attribute | 1233 条声明 / 372 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 372 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，14244 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 182 条子入口、7442 个导出名、128 个 `data-scope` 与 944 条部件配对、
128 个组件的 1587 个 prop 名、221 种 `data-*`、34 个 `data-state` 取值、416 个令牌、
