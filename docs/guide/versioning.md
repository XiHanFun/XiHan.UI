...
`connect` 一共产出 224 个不同的 `data-*` 属性名、1359 条「组件 × 属性」配对。分两类。
...
以入库基线（`ui/tooling/public-surface.json`，15206 个名字）比对当前状态：
基线中有而当前没有，即为删除或改名，构建失败。新增一律放行，因为新增是 minor。

覆盖：包名与 191 条子入口、7899 个导出名、134 个 `data-scope` 与 1008 条部件配对、
134 个组件的 1722 个 prop 名、226 种 `data-*`、33 个 `data-state` 取值、417 个令牌、
