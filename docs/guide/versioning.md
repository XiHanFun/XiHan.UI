...
| 组件覆盖槽 | 3485（覆盖 125 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，13875 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 175 条子入口、7281 个导出名、126 个 `data-scope` 与 940 条部件配对、
126 个组件的 1570 个 prop 名、202 种 `data-*`、32 个 `data-state` 取值、397 个令牌、
5 个 `@layer` 名、3485 个组件覆盖槽、128 个自定义元素及其 attribute 与事件。
