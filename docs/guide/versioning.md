...
| Vue 组件导出 `Xh*` | 719（127 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| `data-part` 取值（部件名） | 218 个不同名字 / 802 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 218 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 270 条 | **只增不减**，方向见下 |
...
**样式钩子**——自带皮肤自己就消费了 127 个属性名 / 552 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 2762（覆盖 116 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| observed attribute | 1065 条声明 / 334 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 334 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
| `CustomEvent` 名 | 77 个名字 / 170 条「元素 × 事件」 | **受约束** |
| 事件传播语义 | `bubbles: true, composed: true`（159 处中 157 处） | **受约束**——把冒泡改掉会让祖先节点上的事件委托静默失效。唯一的例外是 `xh-composer` 的 `submit`：与原生表单提交同名，刻意不冒泡，免得被祖先 `<form>` 当成自己的提交 |
| 事件 `detail` 形状 | 152 个 `*Details` 类型 | **受约束**，等同于 headless 的同名类型 |
| `attribute: false` 的 JS 字段 | 155 条（涉及 56 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 这类只能用 JS 赋值，HTML 里表达不出来——**不是每个 property 都有对应 attribute** |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，10041 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 194 条子入口、4639 个导出名、127 个 `data-scope` 与 802 条部件配对、
127 个组件的 1371 个 prop 名、170 种 `data-*`、20 个 `data-state` 取值、297 个令牌、
5 个 `@layer` 名、2762 个组件覆盖槽、129 个自定义元素及其 attribute 与事件。
