...
- **不要混装版本。** `@xihan-ui/vue` 与 `@xihan-ui/headless` 版本不一致时，类型会对不上；`@xihan-ui/web-components` 更硬——同一个 `xh-` 标签被两个版本注册会直接抛错，整页挂掉。适配器与它的兄弟包目前是普通 `dependencies`，包管理器不会阻止你装出这种组合，所以这一条靠你自己保证（见 [今天兑现不了的](#今天兑现不了的)）。
...
上面的条款里，有一部分**目前只靠人工保证，没有门禁**。列在这里，比假装什么都兜得住有用。

| 条款 | 现状 | 计划补的机制 |
| --- | --- | --- |
| 五种介质的「改名 = major」 | **完全靠自觉。** 仓库里的门禁全是「重新生成 + `git diff --exit-code`」形态，只能发现「改了源忘了跑生成」；改名会在同一个提交里同时改源与产物，diff 干净、CI 全绿。已实测：把某个 `data-part` 改名、把某个令牌改名，7 道相关门禁全部通过 | 一份入库的公开面基线（导出名 / `data-scope` / `data-part` / `data-*` / 令牌 / 层名 / 标签 / attribute / 事件 / 方法）+ 一道「基线里有而当前没有就失败」的门禁 |
| 1861 条组件覆盖槽 | 没有集中清单。判据是自带皮肤里出现过 `var(--xh-<组件>-…)`；组件文档页目前一条 CSS 变量都没列 | 随包发布一份机读的槽位清单，并接入上面的基线门禁 |
| 22 个 `data-state` 取值、6 个 `data-tone` 值 | 词汇表只存在于皮肤的选择器里。JS 侧 109 处把 `tone` / `size` / `variant` 声明成裸 `string`，写错值编译通过、运行期静默无样式 | 三轴收成联合类型（这是收窄，只能在 major 或 1.0 之前做） |
| Vue 事件载荷类型 | 70 个带事件的组件里 69 个用数组式 `emits`，类型产物是 `(...args: any[]) => any`。也就是说事件载荷这一层**今天没有可被违反的契约** | 改成对象式声明并接上 headless 已有的 106 个 `*ChangeDetails` |
...
| Node 下限 | TypeScript 的 `lib` 是 `ES2023` 而 `engines` 是 `>=18`。今天就能写 `toSorted`（Node 20+），类型检查全绿、Node 18 运行时炸 | `lib` 收到 `ES2022` 与 `engines` 对齐 |
| 「14 个包必须同版本」 | 兄弟包是普通 `dependencies` 而非 `peerDependencies`，包管理器不会阻止你装出混版组合 | 提成 peer，或在入口加运行期版本一致性检查 |
| 产物契约（`publint` / `attw`） | 检查脚本的目录遍历只走一层，而包已经按角色分成两级，实际检查了 **0 个包**。发布流水线也不跑门禁 | 修遍历深度、加「扫到 0 个包即失败」的自检、把门禁接进发布流程 |
