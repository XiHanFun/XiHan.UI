...
| Vue 组件导出 `Xh*` | 1006（134 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| `custom-elements.json`（CEM） | 1 份 / 136 个元素 | 已经进过清单的 `tagName` / `attribute` / `event` 条目不会消失；`cssProperties`（皮肤覆盖槽）与 `events` 的 `type`（detail 类型）由 `scripts/enrich-cem.mjs` 从皮肤与元素源码生成，`gate:cem` 校验同步。字段结构细节仍不承诺，补充算 minor |
...
| `xxxAnatomy` / `xxxMeta` / `xxxKeyboard` 三组导出对象 | 各 134 | **它们描述的 part 名单是受约束的（见第二节），但这三个对象本身的组织方式不是。** 想拿到部件名单，请以组件文档页的解剖表为准，不要 import 这些对象 |
...
| WC 的元素类导出 `Xh*Element` | 108 | 只作 `instanceof` 与手动 `customElements.define` 的便利品，**不支持 `extends`**（基类不导出、`wire()` 是 protected abstract）。要拿元素请用 `document.querySelector` |
| WC 元素上的 `static partContract` | 134 | 部件校验的输入数据，实现细节 |
...
| `data-part` 取值（部件名） | 259 个不同名字 / 1008 条「组件 × 部件」配对 | **受约束** |
...
`connect` 一共产出 220 个不同的 `data-*` 属性名、1033 条「组件 × 属性」配对。分两类。
...
| 组件覆盖槽 | 3895（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| observed attribute | 1335 条声明 / 378 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
- 元素在 `@xihan-ui/web-components/define`，不在包主入口。`defineXhElements()` 是全有全无——调它就注册全部 136 个元素，没有逐个的 `defineXhButton()`。（补细粒度 define 是 minor；补完之后 `defineXhElements` 的「全量」语义就成了承诺。）
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 1006 个组件、102 个组合式函数 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，15138 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 190 条子入口、7876 个导出名、134 个 `data-scope` 与 1008 条部件配对、
134 个组件的 1718 个 prop 名、222 种 `data-*`、34 个 `data-state` 取值、419 个令牌、
5 个 `@layer` 名、3895 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
