...
样式钩子。自带皮肤消费了 165 个属性名 / 770 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤参照的就是这一组：
...
::: warning 命名前缀不能反推归属
`--xh-field-py` 形似 `field` 组件的覆盖槽，实际是全局语义令牌，`field.css` 本身并不使用它。同理 `--xh-text-*`（13 个全局文本令牌）与 `text-field` 的 48 条组件槽同前缀，`--xh-color-*`（43 个原语调色板令牌）与 `color-picker` 的 70 条组件槽同前缀。判断一条属性属于哪一档，看它在不在上表列的那 415 个全局令牌里，不按前缀推断。
:::
...
以入库基线（`ui/tooling/public-surface.json`，15160 个名字）比对当前状态：
