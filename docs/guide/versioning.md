...
| `data-part` 取值（部件名） | 201 个不同名字 / 751 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 201 个 | **受约束** |
...
**样式钩子**——自带皮肤自己就消费了 119 个属性名 / 516 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 2615（覆盖 116 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，9475 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 186 条子入口、4376 个导出名、120 个 `data-scope` 与 751 条部件配对、
120 个组件的 1283 个 prop 名、156 种 `data-*`、20 个 `data-state` 取值、294 个令牌、
5 个 `@layer` 名、2615 个组件覆盖槽、122 个自定义元素及其 attribute 与事件。
