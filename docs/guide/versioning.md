...
样式钩子。自带皮肤消费了 169 个属性名 / 716 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤参照的就是这一组：
...
| 组件覆盖槽 | 3929（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，在 `:root` 中设置即可修改该组件 |
...
以入库基线（`ui/tooling/public-surface.json`，15202 个名字）比对当前状态：
...
5 个 `@layer` 名、3929 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
