...
`connect` 一共产出 217 个不同的 `data-*` 属性名、946 条「组件 × 属性」配对。分两类。
...
| 组件覆盖槽 | 3623（覆盖 126 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| observed attribute | 1230 条声明 / 373 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
| `attribute: false` 的 JS 字段 | 193 条（涉及 68 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 这类只能用 JS 赋值，HTML 里表达不出来——**不是每个 property 都有对应 attribute** |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，14234 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 182 条子入口、7448 个导出名、128 个 `data-scope` 与 944 条部件配对、
128 个组件的 1584 个 prop 名、219 种 `data-*`、34 个 `data-state` 取值、416 个令牌、
5 个 `@layer` 名、3623 个组件覆盖槽、130 个自定义元素及其 attribute 与事件。
