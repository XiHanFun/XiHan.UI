...
| Vue 组件导出 `Xh*` | 1011（134 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| `data-part` 取值（部件名） | 260 个不同名字 / 1013 条「组件 × 部件」配对 | **受约束** |
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 1011 个组件、102 个组合式函数 |
...
以入库基线（`ui/tooling/public-surface.json`，15516 个名字）比对当前状态：
基线中有而当前没有，即为删除或改名，构建失败。新增一律放行，因为新增是 minor。

覆盖：包名与 191 条子入口、7952 个导出名、134 个 `data-scope` 与 1013 条部件配对、
