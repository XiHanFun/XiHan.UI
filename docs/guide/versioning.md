...
| Vue 组件导出 `Xh*` | 990（130 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
| Vue 组合式函数 `use<家族>` | 99 | `useSelect`、`useCombobox`。这是「不用我的部件、自己写标记」的唯一入口 |
...
| WC 的元素类导出 `Xh*Element` | 104 | 只作 `instanceof` 与手动 `customElements.define` 的便利品，**不支持 `extends`**（基类不导出、`wire()` 是 protected abstract）。要拿元素请用 `document.querySelector` |
...
| `data-part` 取值（部件名） | 261 个不同名字 / 989 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 261 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 285 条 | **受约束**（加条目 = major），方向见下 |
...
`connect` 一共产出 221 个不同的 `data-*` 属性名、992 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 166 个属性名 / 753 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3817（覆盖 129 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| `@xihan-ui/styles` 的 CSS 子路径 | 150 | `.`、`./index.css`、`./index.unlayered.css`，与 141 条 `.css`——130 份组件皮肤加 `./layers.css`、`./tone.css`、`./reset.css`、`./overlay-arrow.css`、`./visually-hidden.css`、`./undefined.css`、`./focus.css`、`./label.css`、`./description.css`、`./pointer.css`、`./forced-colors.css` |
...
| observed attribute | 1289 条声明 / 374 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 374 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
| `CustomEvent` 名 | 90 个名字 / 197 条「元素 × 事件」 | **受约束** |
| 事件传播语义 | `bubbles: true, composed: true`（182 处中 180 处） | **受约束**——把冒泡改掉会让祖先节点上的事件委托静默失效。例外是名为 `submit` 的事件（`xh-prompt-input` / `xh-question-flow`）：与原生表单提交同名，一律不冒泡，免得被祖先 `<form>` 当成自己的提交 |
| 事件 `detail` 形状 | 181 个 `*Details` 类型 | **受约束**，等同于 headless 的同名类型 |
| `attribute: false` 的 JS 字段 | 205 条（涉及 68 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 这类只能用 JS 赋值，HTML 里表达不出来——**不是每个 property 都有对应 attribute** |
| 命令式方法 | 84（分布在 44 个元素） | **受约束**，含参数与返回类型 |
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 990 个组件、99 个组合式函数 |
| `@xihan-ui/react` | 组件与 hooks（铺开中，公开面随批次增长） |
| `@xihan-ui/web-components` | 133 个自定义元素 |
| `@xihan-ui/headless` | `connect*` / `*Machine` / 各类公开类型；内部算子在排除清单里 |
| `@xihan-ui/styles` | 130 份组件皮肤、5 个层名 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，14858 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 186 条子入口、7747 个导出名、130 个 `data-scope` 与 989 条部件配对、
130 个组件的 1664 个 prop 名、223 种 `data-*`、34 个 `data-state` 取值、418 个令牌、
5 个 `@layer` 名、3817 个组件覆盖槽、132 个自定义元素及其 attribute 与事件。
...
写错值编译期就报错。**Vue 事件载荷也有类型了**，95 个组件的 `emits` 全是对象式，
