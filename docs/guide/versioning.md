...
| Vue 组件导出 `Xh*` | 927（128 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| `data-part` 取值（部件名） | 259 个不同名字 / 944 条「组件 × 部件」配对 | **受约束** |
...
`connect` 一共产出 219 个不同的 `data-*` 属性名、953 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 166 个属性名 / 732 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3634（覆盖 126 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
::: warning 命名前缀不能反推归属
`--xh-field-py` 长得像 `field` 组件的覆盖槽，实际是全局语义令牌，而且 `field.css` 自己都不用它。同理 `--xh-text-*`（13 个全局文本令牌）与 `text-field` 的 48 条组件槽同前缀，`--xh-color-*`（43 个原语调色板令牌）与 `color-picker` 的 70 条组件槽同前缀。判断一条属性属于哪一档，看它在不在上表列的那 415 个全局令牌里，不要按前缀猜。
:::
...
| observed attribute | 1235 条声明 / 373 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 927 个组件、97 个组合式函数 |
...
| `@xihan-ui/tokens` | 415 个令牌名，外加 `./runtime` 的主题控制器与种子色 API |
| `@xihan-ui/icons` | 图标集 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，14241 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 182 条子入口、7438 个导出名、128 个 `data-scope` 与 944 条部件配对、
128 个组件的 1589 个 prop 名、221 种 `data-*`、34 个 `data-state` 取值、415 个令牌、
5 个 `@layer` 名、3634 个组件覆盖槽、130 个自定义元素及其 attribute 与事件。
