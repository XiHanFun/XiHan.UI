...
| Vue 组件导出 `Xh*` | 1007（134 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| `data-part` 取值（部件名） | 259 个不同名字 / 1009 条「组件 × 部件」配对 | **受约束** |
...
其余 `data-xh-*` 属性（`data-xh-scrollbar`、`data-xh-focus-guard`、`data-xh-inert-exempt` 等 37 个）是库自用标记，排除在承诺之外。`data-xh-part` 例外：它是 Web Components 适配器唯一的作者输入 API。`data-xh-scroll` 也由作者书写，但它只挂 reset 层的原生细条样式（见[皮肤与样式分层](./styling#组件内滚动)），不进任何组件契约。作者书写 `data-xh-part="trigger"` 是声明，元素接线后在同一节点写入 `data-scope` + `data-part` 是事实。皮肤匹配后者，后者不应手写。
:::
...
`connect` 一共产出 224 个不同的 `data-*` 属性名、1412 条「组件 × 属性」配对。分两类。
...
样式钩子。自带皮肤消费了 170 个属性名 / 726 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤参照的就是这一组：
...
| 组件覆盖槽 | 3997（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，在 `:root` 中设置即可修改该组件 |
...
| observed attribute | 1337 条声明 / 378 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
以入库基线（`ui/tooling/public-surface.json`，15273 个名字）比对当前状态：
基线中有而当前没有，即为删除或改名，构建失败。新增一律放行，因为新增是 minor。

覆盖：包名与 191 条子入口、7903 个导出名、134 个 `data-scope` 与 1009 条部件配对、
134 个组件的 1721 个 prop 名、226 种 `data-*`、33 个 `data-state` 取值、417 个令牌、
5 个 `@layer` 名、3997 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
