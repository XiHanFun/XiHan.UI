...
**样式钩子**——自带皮肤自己就消费了 154 个属性名 / 701 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3435（覆盖 125 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，13753 个名字）比对当前状态：
...
5 个 `@layer` 名、3435 个组件覆盖槽、128 个自定义元素及其 attribute 与事件。
