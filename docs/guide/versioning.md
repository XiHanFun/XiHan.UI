...
`connect` 一共产出 230 个不同的 `data-*` 属性名、1608 条「组件 × 属性」配对。分两类。
...
样式钩子。自带皮肤消费了 181 个属性名 / 780 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤参照的就是这一组：
...
以入库基线（`ui/tooling/public-surface.json`，16078 个名字）比对当前状态：
