...
| Vue 组件导出 `Xh*` | 719（120 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| 无头内核 `connect*` | 120 | `connectAccordion` 及其参数顺序、返回的 getter 名 |
...
| `custom-elements.json`（CEM） | 1 份 / 122 个元素 | 已经进过清单的 `tagName` / `attribute` / `event` 条目不会消失；`cssProperties`（皮肤覆盖槽）与 `events` 的 `type`（detail 类型）由 `scripts/enrich-cem.mjs` 从皮肤与元素源码生成，`gate:cem` 校验同步。字段结构细节仍不承诺，补充算 minor |
...
| `xxxAnatomy` / `xxxMeta` / `xxxKeyboard` 三组导出对象 | 各 120 | **它们描述的 part 名单是受约束的（见第二节），但这三个对象本身的组织方式不是。** 想拿到部件名单，请以组件文档页的解剖表为准，不要 import 这些对象 |
...
| WC 元素上的 `static partContract` | 120 | 部件校验的输入数据，实现细节 |
...
| 类别 | 数量 | 档位 |
| --- | --- | --- |
| `data-scope` 取值（组件身份） | 120 | **受约束**（新增第 121 个组件是 minor） |
| `data-part` 取值（部件名） | 202 个不同名字 / 752 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 202 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 252 条 | **只增不减**，方向见下 |
...
**样式钩子**——自带皮肤自己就消费了 119 个属性名 / 517 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 2620（覆盖 116 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
::: warning 命名前缀不能反推归属
`--xh-field-py` 长得像 `field` 组件的覆盖槽，实际是全局语义令牌，而且 `field.css` 自己都不用它。同理 `--xh-text-*`（13 个全局文本令牌）与 `text-field` 的 44 条组件槽同前缀，`--xh-color-*`（41 个原语调色板令牌）与 `color-picker` 的 68 条组件槽同前缀。判断一条属性属于哪一档，看它在不在上表列的那 294 个全局令牌里，不要按前缀猜。
:::
...
| 类别 | 数量 | 档位 |
| --- | --- | --- |
| 自定义元素标签 `xh-*` | 123（`defineXhElements()` 注册 122 + `xh-background`） | **受约束** |
| 注册函数 | 2（`defineXhElements`、`defineXhBackground`） | **受约束** |
| observed attribute | 1003 条声明 / 310 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 310 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
| `CustomEvent` 名 | 71 个名字 / 157 条「元素 × 事件」 | **受约束** |
| 事件传播语义 | `bubbles: true, composed: true`（148 处中 147 处） | **受约束**——把冒泡改掉会让祖先节点上的事件委托静默失效。唯一的例外是 `xh-composer` 的 `submit`：与原生表单提交同名，刻意不冒泡，免得被祖先 `<form>` 当成自己的提交 |
| 事件 `detail` 形状 | 141 个 `*Details` 类型 | **受约束**，等同于 headless 的同名类型 |
...
- 元素在 `@xihan-ui/web-components/define`，不在包主入口。`defineXhElements()` 是全有全无——调它就注册全部 122 个元素，没有逐个的 `defineXhButton()`。（补细粒度 define 是 minor；补完之后 `defineXhElements` 的「全量」语义就成了承诺。）
...
| `@xihan-ui/tokens` | 294 个令牌名，外加 `./runtime` 的主题控制器与种子色 API |
| `@xihan-ui/icons` | 图标集 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，9494 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 186 条子入口、4388 个导出名、120 个 `data-scope` 与 752 条部件配对、
120 个组件的 1284 个 prop 名、156 种 `data-*`、20 个 `data-state` 取值、294 个令牌、
5 个 `@layer` 名、2620 个组件覆盖槽、122 个自定义元素及其 attribute 与事件。
