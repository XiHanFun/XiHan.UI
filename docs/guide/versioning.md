...
| Vue 组件导出 `Xh*` | 719（124 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| 无头内核 `connect*` | 124 | `connectAccordion` 及其参数顺序、返回的 getter 名 |
...
| `custom-elements.json`（CEM） | 1 份 / 126 个元素 | 已经进过清单的 `tagName` / `attribute` / `event` 条目不会消失；`cssProperties`（皮肤覆盖槽）与 `events` 的 `type`（detail 类型）由 `scripts/enrich-cem.mjs` 从皮肤与元素源码生成，`gate:cem` 校验同步。字段结构细节仍不承诺，补充算 minor |
...
| `xxxAnatomy` / `xxxMeta` / `xxxKeyboard` 三组导出对象 | 各 124 | **它们描述的 part 名单是受约束的（见第二节），但这三个对象本身的组织方式不是。** 想拿到部件名单，请以组件文档页的解剖表为准，不要 import 这些对象 |
...
| WC 的元素类导出 `Xh*Element` | 95 | 只作 `instanceof` 与手动 `customElements.define` 的便利品，**不支持 `extends`**（基类不导出、`wire()` 是 protected abstract）。要拿元素请用 `document.querySelector` |
| WC 元素上的 `static partContract` | 124 | 部件校验的输入数据，实现细节 |
...
| 类别 | 数量 | 档位 |
| --- | --- | --- |
| `data-scope` 取值（组件身份） | 124 | **受约束**（新增第 125 个组件是 minor） |
| `data-part` 取值（部件名） | 213 个不同名字 / 783 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 213 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 261 条 | **只增不减**，方向见下 |
...
**样式钩子**——自带皮肤自己就消费了 126 个属性名 / 541 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 2703（覆盖 116 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| 类别 | 数量 | 档位 |
| --- | --- | --- |
| 自定义元素标签 `xh-*` | 127（`defineXhElements()` 注册 126 + `xh-background`） | **受约束** |
| 注册函数 | 2（`defineXhElements`、`defineXhBackground`） | **受约束** |
| observed attribute | 1039 条声明 / 326 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 326 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
| `CustomEvent` 名 | 77 个名字 / 165 条「元素 × 事件」 | **受约束** |
| 事件传播语义 | `bubbles: true, composed: true`（156 处中 155 处） | **受约束**——把冒泡改掉会让祖先节点上的事件委托静默失效。唯一的例外是 `xh-composer` 的 `submit`：与原生表单提交同名，刻意不冒泡，免得被祖先 `<form>` 当成自己的提交 |
| 事件 `detail` 形状 | 149 个 `*Details` 类型 | **受约束**，等同于 headless 的同名类型 |
| `attribute: false` 的 JS 字段 | 152 条（涉及 56 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 这类只能用 JS 赋值，HTML 里表达不出来——**不是每个 property 都有对应 attribute** |
...
- 元素在 `@xihan-ui/web-components/define`，不在包主入口。`defineXhElements()` 是全有全无——调它就注册全部 126 个元素，没有逐个的 `defineXhButton()`。（补细粒度 define 是 minor；补完之后 `defineXhElements` 的「全量」语义就成了承诺。）
...
拿一份入库的基线（`ui/tooling/public-surface.json`，9858 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 191 条子入口、4567 个导出名、124 个 `data-scope` 与 783 条部件配对、
124 个组件的 1341 个 prop 名、167 种 `data-*`、20 个 `data-state` 取值、297 个令牌、
5 个 `@layer` 名、2703 个组件覆盖槽、126 个自定义元素及其 attribute 与事件。
