...
| 组件覆盖槽 | 3644（覆盖 126 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，14252 个名字）比对当前状态：
...
5 个 `@layer` 名、3644 个组件覆盖槽、130 个自定义元素及其 attribute 与事件。
