...
| Vue 组件导出 `Xh*` | 719（125 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| WC 的元素类导出 `Xh*Element` | 96 | 只作 `instanceof` 与手动 `customElements.define` 的便利品，**不支持 `extends`**（基类不导出、`wire()` 是 protected abstract）。要拿元素请用 `document.querySelector` |
...
| `data-part` 取值（部件名） | 213 个不同名字 / 786 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 213 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 264 条 | **只增不减**，方向见下 |
...
**样式钩子**——自带皮肤自己就消费了 126 个属性名 / 545 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 2726（覆盖 116 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| observed attribute | 1049 条声明 / 330 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 330 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
| `CustomEvent` 名 | 77 个名字 / 168 条「元素 × 事件」 | **受约束** |
| 事件传播语义 | `bubbles: true, composed: true`（157 处中 155 处） | **受约束**——把冒泡改掉会让祖先节点上的事件委托静默失效。唯一的例外是 `xh-composer` 的 `submit`：与原生表单提交同名，刻意不冒泡，免得被祖先 `<form>` 当成自己的提交 |
| 事件 `detail` 形状 | 151 个 `*Details` 类型 | **受约束**，等同于 headless 的同名类型 |
| `attribute: false` 的 JS 字段 | 153 条（涉及 56 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 这类只能用 JS 赋值，HTML 里表达不出来——**不是每个 property 都有对应 attribute** |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，9921 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 192 条子入口、4589 个导出名、125 个 `data-scope` 与 786 条部件配对、
125 个组件的 1355 个 prop 名、168 种 `data-*`、20 个 `data-state` 取值、297 个令牌、
5 个 `@layer` 名、2726 个组件覆盖槽、127 个自定义元素及其 attribute 与事件。
