...
`connect` 一共产出 225 个不同的 `data-*` 属性名、1543 条「组件 × 属性」配对。分两类。
...
以入库基线（`ui/tooling/public-surface.json`，15330 个名字）比对当前状态：
基线中有而当前没有，即为删除或改名，构建失败。新增一律放行，因为新增是 minor。

覆盖：包名与 191 条子入口、7931 个导出名、134 个 `data-scope` 与 1009 条部件配对、
