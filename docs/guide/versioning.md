...
| Vue 组件导出 `Xh*` | 1005（133 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| `data-part` 取值（部件名） | 259 个不同名字 / 1007 条「组件 × 部件」配对 | **受约束** |
...
`connect` 一共产出 220 个不同的 `data-*` 属性名、1031 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 165 个属性名 / 769 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| observed attribute | 1326 条声明 / 374 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 1005 个组件、102 个组合式函数 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，15102 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 189 条子入口、7857 个导出名、133 个 `data-scope` 与 1007 条部件配对、
133 个组件的 1709 个 prop 名、222 种 `data-*`、34 个 `data-state` 取值、419 个令牌、
