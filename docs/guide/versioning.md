...
`connect` 一共产出 224 个不同的 `data-*` 属性名、1424 条「组件 × 属性」配对。分两类。
...
样式钩子。自带皮肤消费了 170 个属性名 / 728 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤参照的就是这一组：
...
| 组件覆盖槽 | 4000（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，在 `:root` 中设置即可修改该组件 |
...
以入库基线（`ui/tooling/public-surface.json`，15277 个名字）比对当前状态：
基线中有而当前没有，即为删除或改名，构建失败。新增一律放行，因为新增是 minor。

覆盖：包名与 191 条子入口、7904 个导出名、134 个 `data-scope` 与 1009 条部件配对、
134 个组件的 1721 个 prop 名、226 种 `data-*`、33 个 `data-state` 取值、417 个令牌、
5 个 `@layer` 名、4000 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
