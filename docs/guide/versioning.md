...
`connect` 一共产出 223 个不同的 `data-*` 属性名、1213 条「组件 × 属性」配对。分两类。
...
样式钩子。自带皮肤消费了 169 个属性名 / 725 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤参照的就是这一组：
...
| 组件覆盖槽 | 3888（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，在 `:root` 中设置即可修改该组件 |
...
| observed attribute | 1339 条声明 / 378 个不同名字 | **受约束**（具体元素上的具体属性名） |
...
覆盖：包名与 191 条子入口、7899 个导出名、134 个 `data-scope` 与 1008 条部件配对、
134 个组件的 1723 个 prop 名、225 种 `data-*`、33 个 `data-state` 取值、417 个令牌、
5 个 `@layer` 名、3888 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
