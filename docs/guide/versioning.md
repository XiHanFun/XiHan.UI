...
`connect` 一共产出 222 个不同的 `data-*` 属性名、1135 条「组件 × 属性」配对。分两类。
...
样式钩子。自带皮肤消费了 167 个属性名 / 742 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤参照的就是这一组：
...
以入库基线（`ui/tooling/public-surface.json`，15157 个名字）比对当前状态：
