...
| Vue 组件导出 `Xh*` | 997（131 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
| Vue 组合式函数 `use<家族>` | 100 | `useSelect`、`useCombobox`。这是「不用我的部件、自己写标记」的唯一入口 |
| Vue 指令 | 2 | `vBackground`（`@xihan-ui/vue/backgrounds`）、`vSound`（`@xihan-ui/vue/sound`），两个子入口各依赖一个可选 peer |
| 无头内核 `connect*` | 131 | `connectAccordion` 及其参数顺序、返回的 getter 名 |
...
| `custom-elements.json`（CEM） | 1 份 / 133 个元素 | 已经进过清单的 `tagName` / `attribute` / `event` 条目不会消失；`cssProperties`（皮肤覆盖槽）与 `events` 的 `type`（detail 类型）由 `scripts/enrich-cem.mjs` 从皮肤与元素源码生成，`gate:cem` 校验同步。字段结构细节仍不承诺，补充算 minor |

### 排除

| 类别 | 数量 | 为什么排除是安全的 |
| --- | --- | --- |
| `@xihan-ui/headless` 的内部算子与常量 | 625 | `clampRating`、`buildMonthGrid`、`colorHexToRgba`、`CAROUSEL_AUTOPLAY_INTERVAL` 这类。它们是内核实现的一部分，改一次实现就得改一次签名。你需要的默认值应当从组件 props 的文档默认值读，不要 import 一个常量再自己比对 |
| `@xihan-ui/headless` 的内部伴生类型 | 564 | `*Refs`（机器持有的 DOM 引用袋，45 个）、`ColorHsva`、`CascaderLevel` 等，是上面那批函数的参数与返回类型 |
| `xxxAnatomy` / `xxxMeta` / `xxxKeyboard` 三组导出对象 | 各 131 | **它们描述的 part 名单是受约束的（见第二节），但这三个对象本身的组织方式不是。** 想拿到部件名单，请以组件文档页的解剖表为准，不要 import 这些对象 |
| Vue 的 `provide*` / `use*Context` 函数 | 111 | [Vue 适配器](../adapters/vue) 早已写明「父子组件之间的 provide / inject 是内部实现，不对外开放」。要下探请用 `use<家族>()` |
...
| WC 的元素类导出 `Xh*Element` | 105 | 只作 `instanceof` 与手动 `customElements.define` 的便利品，**不支持 `extends`**（基类不导出、`wire()` 是 protected abstract）。要拿元素请用 `document.querySelector` |
| WC 元素上的 `static partContract` | 131 | 部件校验的输入数据，实现细节 |
...
| 类别 | 数量 | 档位 |
| --- | --- | --- |
| `data-scope` 取值（组件身份） | 131 | **受约束**（新增第 132 个组件是 minor） |
| `data-part` 取值（部件名） | 261 个不同名字 / 996 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 261 个 | **受约束** |
| `meta.requiredParts`（必备部件） | 288 条 | **受约束**（加条目 = major），方向见下 |
...
其余 `data-xh-*` 属性（`data-xh-scrollbar`、`data-xh-focus-guard`、`data-xh-inert-exempt` 等 34 个）是库自用标记，**排除**在承诺之外。但 `data-xh-part` 不是——它是 Web Components 适配器唯一的作者输入 API：你写 `data-xh-part="trigger"` 是**声明**，元素接线后往同一节点打上 `data-scope` + `data-part` 是**事实**。皮肤匹配后者，你永远不该手写后者。
:::
...
- 删条目：任何版本都可以。
- 加条目：是破坏性变更，只出现在 major，并且从落地那一刻起就按 `error` 报——缺必需角色节点恒发 `error`，没有先以 `warn` 缓一个版本的过渡档。

---

## 三、`data-*` 状态属性

`connect` 一共产出 221 个不同的 `data-*` 属性名、1000 条「组件 × 属性」配对。分两类。
...
**样式钩子**——自带皮肤自己就消费了 166 个属性名 / 760 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤照着抄的就是这一组：
...
| 组件覆盖槽 | 3842（覆盖 130 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，你在 `:root` 里设它就改了这个组件 |
...
| `@xihan-ui/styles` 的 CSS 子路径 | 151 | `.`、`./index.css`、`./index.unlayered.css`，与 142 条 `.css`——131 份组件皮肤加 `./layers.css`、`./tone.css`、`./reset.css`、`./overlay-arrow.css`、`./visually-hidden.css`、`./undefined.css`、`./focus.css`、`./label.css`、`./description.css`、`./pointer.css`、`./forced-colors.css` |
...
::: warning 命名前缀不能反推归属
`--xh-field-py` 长得像 `field` 组件的覆盖槽，实际是全局语义令牌，而且 `field.css` 自己都不用它。同理 `--xh-text-*`（13 个全局文本令牌）与 `text-field` 的 48 条组件槽同前缀，`--xh-color-*`（43 个原语调色板令牌）与 `color-picker` 的 70 条组件槽同前缀。判断一条属性属于哪一档，看它在不在上表列的那 418 个全局令牌里，不要按前缀猜。
:::
...
| 类别 | 数量 | 档位 |
| --- | --- | --- |
| 自定义元素标签 `xh-*` | 134（`defineXhElements()` 注册 133 + `xh-background`） | **受约束** |
| 注册函数 | 2（`defineXhElements`、`defineXhBackground`） | **受约束** |
| observed attribute | 1301 条声明 / 375 个不同名字 | **受约束**（具体元素上的具体属性名） |
| attribute 名词汇表本身 | 375 | **只增不减**（新组件复用 `size` / `tone` / `dir` 不算破坏） |
| `CustomEvent` 名 | 90 个名字 / 199 条「元素 × 事件」 | **受约束** |
| 事件传播语义 | `bubbles: true, composed: true`（184 处中 182 处） | **受约束**——把冒泡改掉会让祖先节点上的事件委托静默失效。例外是名为 `submit` 的事件（`xh-prompt-input` / `xh-question-flow`）：与原生表单提交同名，一律不冒泡，免得被祖先 `<form>` 当成自己的提交 |
| 事件 `detail` 形状 | 182 个 `*Details` 类型 | **受约束**，等同于 headless 的同名类型 |
| `attribute: false` 的 JS 字段 | 206 条（涉及 68 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 这类只能用 JS 赋值，HTML 里表达不出来——**不是每个 property 都有对应 attribute** |
| 命令式方法 | 85（分布在 45 个元素） | **受约束**，含参数与返回类型 |
...
- 元素在 `@xihan-ui/web-components/define`，不在包主入口。`defineXhElements()` 是全有全无——调它就注册全部 133 个元素，没有逐个的 `defineXhButton()`。（补细粒度 define 是 minor；补完之后 `defineXhElements` 的「全量」语义就成了承诺。）
...
**本库不设废弃期，也不留兼容层。** 名字要改就直接改、东西要删就直接删：库里不会为旧写法保留别名、不会把旧名塞进兜底链、也不会在源码里挂 `@deprecated` 让它多活几个版本。你在一个 major 里读到的名字，就是当时唯一存在的那个。

移除动作集中在 major，一律出现在更新日志顶部、单列一节，逐条写清楚旧名是什么、换成什么——这就是唯一的迁移材料。

有一种介质值得单独说：`data-*`、CSS 自定义属性、`@layer` 名、元素 attribute 这四样**没有 IDE 提示**，改名之后你那条选择器只会静默失配，不报错也不降级。这四样唯一的告知渠道就是更新日志，所以每次移除都在 changeset 里把旧名与替换写法逐条列全，你可以直接照着在自己的代码库里全文搜索。
...
下面两张表覆盖全部 17 个包，每个包必在其中一张里；门禁比对两张表的包名与 `packages/` 下的公开包，新增一个包不定级就构建失败。

### 稳定

破坏性变更只出现在 major，移除走上面的流程。

| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 997 个组件、100 个组合式函数 |
| `@xihan-ui/react` | 组件与 hooks（铺开中，公开面随批次增长） |
| `@xihan-ui/web-components` | 134 个自定义元素 |
| `@xihan-ui/headless` | `connect*` / `*Machine` / 各类公开类型；内部算子在排除清单里 |
| `@xihan-ui/styles` | 131 份组件皮肤、5 个层名 |
| `@xihan-ui/tokens` | 418 个令牌名，外加 `./runtime` 的主题控制器与种子色 API |
| `@xihan-ui/icons` | 图标集 |
...
拿一份入库的基线（`ui/tooling/public-surface.json`，14950 个名字）比对当前状态：
**基线里有而当前没有，就是删了或改名了，构建失败**。新增一律放行，因为那是 minor。

覆盖：包名与 187 条子入口、7792 个导出名、131 个 `data-scope` 与 996 条部件配对、
131 个组件的 1679 个 prop 名、223 种 `data-*`、34 个 `data-state` 取值、418 个令牌、
5 个 `@layer` 名、3842 个组件覆盖槽、133 个自定义元素及其 attribute 与事件。
...
写错值编译期就报错。**Vue 事件载荷也有类型了**，96 个组件的 `emits` 全是对象式，
