...
| Vue 组件导出 `Xh*` | 1040（134 个家族） | `XhButton`、`XhSelectRoot`、`XhSelectItemIndicator` |
...
| WC 的元素类导出 `Xh*Element` | 108 | 仅用于 `instanceof` 与手动 `customElements.define`，不支持 `extends`（基类不导出、`wire()` 是 protected abstract）。获取元素使用 `document.querySelector` |
...
| `data-part` 取值（部件名） | 263 个不同名字 / 1042 条「组件 × 部件」配对 | **受约束** |
| `data-xh-part`（WC 作者书写的角色声明） | 属性名 1 个，取值即上面 263 个 | **受约束** |
...
`connect` 一共产出 230 个不同的 `data-*` 属性名、1598 条「组件 × 属性」配对。分两类。
...
样式钩子。自带皮肤消费了 181 个属性名 / 773 条「皮肤 × 属性」配对（不含解剖的 `data-scope` / `data-part`），第三方皮肤参照的就是这一组：
...
| 组件覆盖槽 | 4045（覆盖 133 个组件） | `--xh-button-bg`、`--xh-button-h`、`--xh-dialog-max-w`。全部写成 `var(--xh-x-y, 默认值)` 形态，在 `:root` 中设置即可修改该组件 |
...
| `attribute: false` 的 JS 字段 | 210 条（涉及 70 个字段名） | **受约束**。`collection`、`translations`、`validate`、`filter` 等只能通过 JS 赋值，HTML 中无法表达：不是每个 property 都有对应 attribute |
...
- 元素在 `@xihan-ui/web-components/define`，不在包主入口。`defineXhElements()` 是全量注册：调用即注册全部 136 个元素，没有逐个的 `defineXhButton()`。（补充细粒度 define 是 minor；补充后 `defineXhElements` 的全量语义即成为承诺。）
...
| 包 | 说明 |
| --- | --- |
| `@xihan-ui/vue` | 1040 个组件、102 个组合式函数 |
...
以入库基线（`ui/tooling/public-surface.json`，16059 个名字）比对当前状态：
基线中有而当前没有，即为删除或改名，构建失败。新增一律放行，因为新增是 minor。

覆盖：包名与 194 条子入口、8362 个导出名、134 个 `data-scope` 与 1042 条部件配对、
134 个组件的 1720 个 prop 名、232 种 `data-*`、33 个 `data-state` 取值、658 个令牌、
5 个 `@layer` 名、4045 个组件覆盖槽、136 个自定义元素及其 attribute 与事件。
...
写错取值在编译期报错。Vue 事件载荷已有类型，98 个组件的 `emits` 全是对象式，
