...
::: warning 命名前缀不能反推归属
`--xh-field-py` 形似 `field` 组件的覆盖槽，实际是全局语义令牌，`field.css` 本身并不使用它。同理 `--xh-text-*`（13 个全局文本令牌）与 `text-field` 的 48 条组件槽同前缀，`--xh-color-*`（43 个原语调色板令牌）与 `color-picker` 的 70 条组件槽同前缀。判断一条属性属于哪一档，看它在不在上表列的那 653 个全局令牌里，不按前缀推断。
:::
...
| `@xihan-ui/tokens` | 653 个令牌名，外加 `./runtime` 的主题控制器与种子色 API |
| `@xihan-ui/icons` | 图标集 |
...
以入库基线（`ui/tooling/public-surface.json`，16032 个名字）比对当前状态：
基线中有而当前没有，即为删除或改名，构建失败。新增一律放行，因为新增是 minor。

覆盖：包名与 194 条子入口、8349 个导出名、134 个 `data-scope` 与 1040 条部件配对、
134 个组件的 1720 个 prop 名、232 种 `data-*`、33 个 `data-state` 取值、653 个令牌、
