...
不能用于生产。库包尚未发布到 npm，版本还是 `0.0.0`，无障碍存量问题也没修完（17 个组件加一条全局对比度问题）。

能做的是：克隆仓库跑 playground 看行为、读源码、或者在实验项目里链接试用。见[安装与接入](./installation)。

## 为什么 `npm install` 装不到

因为还没发布。发布走 changesets，全部库包同属一个 fixed 版本组，届时会一起升到同一版本号。
...
**69 个**。每个组件同时有无头内核、Vue 组件、自定义元素与默认皮肤四份产物，本文档的[组件总览](./components/)按分类列全，每个组件一页。

仓库 README 里写的 65 是旧数字，后来补了 `alert` / `spinner` / `skeleton` / `empty-state` 四个。
...
1. `packages/headless/src/<name>/`——解剖、状态机、`connect`、键盘规格表、元数据；
2. `packages/vue/src/components/<name>/`——组件与组合式函数；
3. `packages/wc/src/elements/<name>.ts`——自定义元素，并在 `define.ts` 注册；
4. `packages/styled/styles/<name>.css`——皮肤，并在 `index.css` 引入。
...
没有承诺的时间表。当前状态诚实记录在[无障碍与键盘规格](./guide/a11y#存量违规登记表)：17 个组件的具体违规、一条全局对比度问题、四条步骤重放豁免（其中三条是待查的疑似缺陷）。
...
- 两个 playground 覆盖全部 69 个组件，行为问题优先在那里复现
