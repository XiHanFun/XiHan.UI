...
XiHan.UI 的 18 个公开包按 `1.0.0` 锁步发布到 npm，唯一例外是 `@xihan-ui/pointer`（尚未发布）。当前版本号以 npm 为准：`npm view @xihan-ui/vue version`。
...
1. **克隆仓库直接开发**——构建库包后把文档站跑起来，121 个组件的示例都是真实组件；
...
全量是 127 份皮肤加令牌，压缩后约 66 kB gzip。没有明确的体积压力就用第一种。
:::
