...
| `data-xh-ink="auto"` + `--xh-ink-surface` | 由底色以相对颜色语法按相对亮度 0.179 选墨色（与 `tone.css` 的黑白字分界同一个数）；写在 `@supports` 内，不支持时等于未声明 |
...
- 弹簧只用于三处：手势松手（缺省）、切换与指示（liquid 档）。其余角色用曲线：出现与披露经 Presence 等待 `animationend`，弹簧没有确定的结束时间；数值必须精确落在终值。
- 适配器中不写动画代码。
...
| §7.7 墨色域与缺省面墨色 | 令牌构建按对比度等价生成墨色比例；`data-xh-ink` / `-margin` 重映射；库自有彩色面接入；缺省面改墨色表达前先审计叠边与淡底叠淡底 |
