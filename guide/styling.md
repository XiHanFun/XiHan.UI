来源：https://ui.docs.xihanfun.com/guide/styling
...
下拉、日期、文本框等控件默认带 `min-inline-size: var(--xh-control-min-w)`，默认值 `12rem`（约 192px）。控件放进 flex 或 grid 中会被压缩，压缩到只剩箭头时无法使用，因此提供一条底线。

20 份皮肤消费这条令牌（其中 file-upload 用它作为条目内文件名的下限，不是控件宽度）。窄栏场景（两列表单、抽屉内的设置项、表格上方的筛选行）可能超出容器：这条令牌是底线不是宽度，容器再窄也不让步。可以放开它，或为控件所在的栏预留足够宽度：
...
槽名是 `--xh-<组件名>-control-min-w`。`text-field` · `password-input` · `clipboard` · `mention` 的内层输入框另有一条 `--xh-<组件名>-input-min-w`：外框放开后内框仍然撑开，说明缺少这一条。完整列表见各组件页的“CSS 变量”。
