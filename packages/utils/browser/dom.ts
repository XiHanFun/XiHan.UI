...
export const domUtils = {
   ...
   *
   * @param el 要操作的元素
   * @param cls 要切换的类名，可以是单个类名或多个类名以空格分隔的字符串
   */
  toggleClass: (el: Element, cls: string): void => {
    // 检查元素或类名是否为空，如果为空则直接返回
    if (!el || !cls.trim()) return;

    // 如果元素包含类名，则移除它，否则添加它
    domUtils.hasClass(el, cls) ? domUtils.removeClass(el, cls) : domUtils.addClass(el, cls);
  },
};
