...
Changes must pass the full CI gate, and CI runs the same commands you do locally: `pnpm lint`, `pnpm typecheck`, `pnpm boundaries`, `pnpm gate` (one command runs 97 structural checks), `pnpm test`, `pnpm build`, `pnpm size` and more.
