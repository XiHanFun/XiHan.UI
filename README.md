...
17 个公开包均已发布至 npm，当前版本 `1.0.0-alpha.2`（`latest` 与 `alpha` 两个 tag 同指这一版）。
...
改动需通过 CI 全套门禁，CI 与本地同一套命令：`pnpm lint`、`pnpm typecheck`、`pnpm boundaries`、`pnpm gate`（一条命令跑 57 项结构检查）、`pnpm test`、`pnpm build`、`pnpm size` 等。包一览、分层拓扑与命名约定见 [ui/README_cn.md](./ui/README_cn.md)。
