...
<p>
  ...
  <img alt="Components" src="https://img.shields.io/badge/Components-132-1f6feb?style=flat-square" />
...
- **132 components** - covering general, layout, navigation, data entry, data display, feedback, overlay and AI chat — eight groups
...
Changes must pass the full CI gate, and CI runs the same commands you do locally: `pnpm lint`, `pnpm typecheck`, `pnpm boundaries`, `pnpm gate` (one command runs 113 structural checks), `pnpm test`, `pnpm build`, `pnpm size` and more.

## Scope

In the box: 132 components with their cores and three adapters, 127 visual-component skins, design tokens and the theme runtime, the cross-adapter conformance suite, the accessibility sweep and floating-position contract in real Chromium, and the documentation site.
