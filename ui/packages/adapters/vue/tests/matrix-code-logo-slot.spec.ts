...
// 直接指到组件目录：XhMatrixCodeLogo 的包级导出由接线一并补，测试不等它
import { XhMatrixCode, XhMatrixCodeLogo } from '../src/components/matrix-code/matrix-code'

function render(slot?: () => unknown) {
  return mount(XhMatrixCode, {
...
describe('xhMatrixCode 的 logo 插槽', () => {
  ...
  it('真放了 XhMatrixCodeLogo 才挖空，且挖空排在两条 path 之后、logo 之前', () => {
    const wrapper = render(() => [h(XhMatrixCodeLogo, null, () => [h('rect')])])
