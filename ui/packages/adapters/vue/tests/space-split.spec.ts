...
import { XhSpace, XhSpaceSplit } from '../src/components/space/space'

afterEach(() => {
  document.body.innerHTML = ''
})

/** 挂一个 Space，回它 root 上的子元素序列：分隔符记 'split'，其余记标签名。 */
function marks(children: () => VNode[], split?: () => VNode[]): string[] {
  const slots: Record<string, () => VNode[]> = { default: children }
  if (split)
    slots.split = split
  mount(defineComponent({ setup: () => () => h(XhSpace, null, slots) }), { attachTo: document.body })
  const root = document.querySelector('[data-scope="space"][data-part="root"]')!
...
describe('space 分隔符部件', () => {
  it('分隔符是 span 角色节点，恒带 aria-hidden，插槽内容原样落在里面', () => {
    mount(
      defineComponent({
        setup: () => () => h(XhSpace, null, { default: () => [item('甲'), item('乙')], split: () => [h('i', '·')] }),
      }),
      { attachTo: document.body },
    )
    const split = document.querySelector('[data-scope="space"][data-part="split"]')!
  ...
  it('手写分隔符部件与 split 插槽铺出同一种结构：结构一致才谈得上换写法', () => {
    ...
    mount(
      defineComponent({
        setup: () => () => h(XhSpace, null, {
          default: () => [item('甲'), h(XhSpaceSplit, null, () => [h('i')]), item('乙')],
        }),
      }),
      { attachTo: document.body },
    )
