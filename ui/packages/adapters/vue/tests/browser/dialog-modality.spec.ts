...
function backdrop(): HTMLElement | null {
  return document.querySelector(`[data-scope='dialog'][data-part='backdrop']`)
}

function positioner(): HTMLElement {
  return document.querySelector(`[data-scope='dialog'][data-part='positioner']`)!
}

function content(): HTMLElement {
  return document.querySelector(`[data-scope='dialog'][data-part='content']`)!
}
...
function mount(modal: Ref<boolean>): HTMLButtonElement {
  ...
  app = createApp({
    render: () => h(XhDialogRoot, { open: true, modal: modal.value }, () =>
      h(XhDialogContent, null, () => [h(XhDialogTitle, null, () => '设置'), h('button', '保存')])),
  })
  app.mount(host)
  return outside
}
