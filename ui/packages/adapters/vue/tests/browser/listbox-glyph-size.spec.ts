...
function item(value: string): HTMLElement {
  const element = host?.querySelector<HTMLElement>(`[data-scope='listbox'][data-part='item'][data-value='${value}']`)
  if (!element)
    throw new Error(`缺少 listbox 条目：${value}`)
  return element
}

function indicatorOf(value: string): HTMLElement {
  const element = item(value).querySelector<HTMLElement>('[data-scope=\'listbox\'][data-part=\'item-indicator\']')
  if (!element)
...
async function mount(density: 'comfortable' | 'compact'): Promise<void> {
  ...
  app = createApp({
    ...
    render: () => h(XhListboxRoot, { selectionMode: 'multiple', defaultValue: ['apple', 'banana'] }, () => [
      h(XhListboxContent, null, () => [
        h(XhListboxItem, { value: 'apple' }, () => [
          h(XhListboxItemText, null, () => '苹果'),
          h(XhListboxItemIndicator),
        ]),
        h(XhListboxItem, { value: 'banana' }, () => [
          authorIcon('条目图标'),
          h(XhListboxItemText, null, () => '香蕉'),
          h(XhListboxItemIndicator, null, () => authorIcon('标记位图标')),
        ]),
        h(XhListboxItem, { value: 'cherry' }, () => [
          h(XhListboxItemText, null, () => '樱桃'),
          h(XhListboxItemIndicator),
        ]),
      ]),
    ]),
...
describe.each(['comfortable', 'compact'] as const)('列表框自绘状态字形按指示符档取尺（%s）', (density) => {
