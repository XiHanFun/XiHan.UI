...
function mountCalendar(): void {
  ...
  app = createApp({
    setup: () => () => h(XhCalendarPickerRoot, { locale: 'zh-CN', timeZone: 'UTC', defaultValue: '2026-07-15' }, () => [
      h(XhCalendarPickerHeader, null, () => [
        h(XhCalendarPickerPrevYearTrigger),
        h(XhCalendarPickerPrevTrigger),
        h(XhCalendarPickerHeading),
        h(XhCalendarPickerNextTrigger),
        h(XhCalendarPickerNextYearTrigger),
      ]),
    ]),
  })
  app.mount(host)
...
function nav(part: string): HTMLElement {
  const el = document.querySelector<HTMLElement>(`[data-scope="calendar-picker"][data-part="${part}"]`)
  if (!el)
