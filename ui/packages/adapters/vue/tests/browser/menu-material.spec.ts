        ...
        h(XhMenuContent, { style: { inlineSize: '260px' } }, () => [
          h(XhMenuGroup, { value: 'actions' }, () => [
            ...
            h(XhMenuItem, { value: 'detail' }, () => [
              ...
              h(XhMenuItemDescription, { 'data-testid': 'description' }, () => '说明文字独占第二行'),
            ]),
            ...
            h(XhMenuItem, { 'value': 'submenu', 'data-testid': 'submenu' }, () => [
              ...
              h(XhMenuItemDescription, { 'data-testid': 'submenu-description' }, () => '子菜单说明'),
            ]),
...
describe('menu M2 与条目几何', () => {
  ...
  it('正文按剩余宽度截断，只有说明部件换到下一行', async () => {
    await mountMenu()
    ...
    expect(description.getBoundingClientRect().left).toBeCloseTo(raw.getBoundingClientRect().left, 0)
    expect(label.getBoundingClientRect().left).toBeCloseTo(raw.getBoundingClientRect().left, 0)
    expect(text.scrollWidth).toBeGreaterThan(text.clientWidth)
    expect(getComputedStyle(text).overflow).toBe('hidden')
  })

  it('子菜单箭头在行尾，展开项不出现侧条或品牌蓝底，字重保持稳定', async () => {
    await mountMenu()
    const submenu = byTestId('submenu')
    const arrow = getComputedStyle(submenu, '::after')
    expect(getComputedStyle(submenu).display).toBe('flex')
    expect(arrow.flexShrink).toBe('0')
    expect(arrow.order).toBe('1')
    expect(getComputedStyle(byTestId('submenu-description')).order).toBe('2')
    ...
    neutral.style.backgroundColor = 'var(--xh-bg-subtle)'
    submenu.append(neutral)
    expect(getComputedStyle(submenu).backgroundColor).toBe(getComputedStyle(neutral).backgroundColor)
