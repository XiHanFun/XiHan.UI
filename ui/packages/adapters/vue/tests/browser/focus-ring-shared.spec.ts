...
async function focusVisible(scope: string, part: string, tag = 'button', attrs: Record<string, string> = {}): Promise<HTMLElement> {
...
describe('聚焦环公共层', () => {
  it('默认档：任何库节点键盘落焦都出环，粗细与颜色走令牌，环画在元素自己那一圈', async () => {
  ...
  it('换环色档：面是实心的，环改取面自己的前景色，不再是品牌色', async () => {
    ...
    expect(style.outlineColor).not.toBe(ringFocusColor())
  })
