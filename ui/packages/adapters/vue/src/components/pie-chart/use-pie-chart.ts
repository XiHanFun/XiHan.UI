...
export function usePieChart(
  props: PieChartSchema['props'],
  notify?: PieChartNotifiers,
): PieChartContext {
  const rootRef = ref<HTMLElement | null>(null)
  const viewportRef = ref<HTMLElement | null>(null)
  // 传响应式 props 对象本身而非快照，供机器每次读时重新展开
  const service = useMachine(pieChartMachine, () => ({ ...props, ...notify }))
  service.refs.set('getRootEl', () => rootRef.value)
  service.refs.set('getViewportEl', () => viewportRef.value)
  const api = computed(() => connectPieChart(service, vueNormalize))
