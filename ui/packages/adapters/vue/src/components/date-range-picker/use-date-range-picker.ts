...
export interface DateRangePickerContext {
  api: ComputedRef<DateRangePickerApi>
...
export function assertDateRangePickerRootDocument(expected: Document, node: Element): void {
  if (node.ownerDocument !== expected)
    throw new Error('[xh] DateRangePicker 根节点不能在运行期切换 Document')
}

/** 显式 Portal 是同 realm 目标，不是 Scope 来源；字段存在时 null 与伪节点都属于配置错误。 */
export function resolveDateRangePickerPortalTarget(
  ...
  if (!isElement(target))
    throw new TypeError('[xh] DateRangePicker 的 portalContainer 必须返回 Element')
  if (target.ownerDocument !== runtime.layerRegistry.ownerDocument)
    throw new Error('[xh] DateRangePicker 的 portalContainer 必须与组件根属于同一 Document')
  return target
}

function createDateRangePickerContext(
  props: DateRangePickerSchema['props'],
  handlers: DateRangePickerHandlers,
  ...
  const root = useMachine(dateRangePickerMachine, () => ({ ...props, ...handlers }), scope)
  const calendar = useMachine<CalendarRangePickerSchema>(calendarRangePickerMachine, () => dateRangePickerCalendarProps(root), scope)
  ...
  if (typeof document !== 'undefined') {
    const installConfig = (node?: HTMLElement): void => {
      ...
      if (current) {
        ...
        if (node)
          assertDateRangePickerRootDocument(runtimeDocument!, node)
        return
      }
  ...
  const api = computed(() => connectDateRangePicker(services, vueNormalize))
  ...
  const portalTarget = computed<string | Element>(() => {
    ...
    if (!runtime)
      return 'body'
    const explicitTarget = resolveDateRangePickerPortalTarget(runtime, xhConfig.value.portalContainer)
    if (explicitTarget)
      return explicitTarget
    const defaultTarget = runtime.portalContainer()
    if (!isElement(defaultTarget))
      throw new TypeError('[xh] DateRangePicker 的运行时 portalContainer 必须返回 Element')
...
export function useDateRangePicker(
  ...
  return createDateRangePickerContext(props, handlers)
}

/** 组合组件内部从真实根节点绑定 Scope；不经包入口导出。 */
export function useDateRangePickerWithRoot(
  props: DateRangePickerSchema['props'],
  handlers: DateRangePickerHandlers,
  rootRef: Readonly<Ref<HTMLElement | null>>,
): DateRangePickerContext {
  return createDateRangePickerContext(props, handlers, rootRef)
}
