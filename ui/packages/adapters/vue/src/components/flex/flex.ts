...
/**
 * 分隔符部件。写 `split` 插槽时组件在每道缝隙中自动铺设一个，
 ...
 * 把默认插槽摊平为一个个子项。
 *
 * `v-for` 产出的是一个片段而不是若干节点，不摊平就整段算一个子项，分隔符只会插在整段的两端；
 * `v-if` 为假留下的注释节点与模板缩进留下的空白文本不渲染任何像素，保留会插出两条相邻的分隔符。
 */
...
/**
 * 在每两个子项之间铺设一个分隔符部件。
 * 插槽逐缝隙重新求值：同一份 vnode 挂在两处会被后一处替换。
 */
...
export const XhFlex = defineComponent({
  ...
  slots: Object as SlotsType<{
    /** 子项，按写入的顺序排列。 */
    default?: () => VNode[]
    /** 分隔符的内容：写了它，组件在每两个子项之间各铺设一个分隔符部件，逐缝隙重新求值一次。 */
    split?: () => VNode[]
  }>,
  setup(props, { slots }) {
