...
// 验证 drawer modality 相关行为。

import type { XhDrawerElement } from '../../src/elements/drawer'
...
function part(name: string): HTMLElement {
  return document.querySelector(`[data-scope='drawer'][data-part='${name}']`)!
}

function mount(): { drawer: XhDrawerElement, outside: HTMLButtonElement } {
  const outside = outsideButton()
  const host = document.createElement('div')
  host.innerHTML = `
    <xh-drawer open modal="false">
  ...
  return { drawer: host.firstElementChild as XhDrawerElement, outside }
}
