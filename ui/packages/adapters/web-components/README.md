...
- **Presence 模型不同（已知差异，非缺陷）**：Vue 用 Presence 卸载 content（关闭即从 DOM 移除）；
  ...
  （Vue 无 content 节点，WC 有 content[data-state=closed]）。逐帧 parity 覆盖 106 个套件，
  收不进来的 29 个逐条登记在 `tooling/testing/runners/parity.spec.ts` 的 `EXCLUDED` 里并各带理由；
  dialog 在这 29 个里，理由就是这条 presence 模型差异——它属永久性差异，跨适配器保证由两端
