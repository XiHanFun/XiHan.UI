...
import type {
  ...
  CalendarPickerTranslations,
  CalendarPickerValueChangeDetails,
  CalendarView,
  CalendarViewChangeDetails,
  CalendarWeekDay,
  CalendarWeekdayFormat,
} from '@xihan-ui/headless'
import { calendarPickerAnatomy, calendarPickerMachine, calendarPickerMeta, connectCalendarPicker } from '@xihan-ui/headless'
...
export class XhCalendarPickerElement extends XhElement {
  ...
  declare translations?: Partial<CalendarPickerTranslations>

  private readonly notifyValue = (details: CalendarPickerValueChangeDetails): void => {
  ...
  private readonly ctrl = new MachineController<CalendarPickerSchema>(
    this,
  ...
  private machineProps(): Partial<CalendarPickerSchema['props']> {
    return {
      value: this.value,
  ...
  get periods(): CalendarPeriod[] {
    return this.ctrl.service ? connectCalendarPicker(this.ctrl.service, wcNormalize).periods : []
  }
  ...
  protected wire(): void {
    const api = connectCalendarPicker(this.ctrl.service, wcNormalize)
