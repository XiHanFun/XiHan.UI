...
import type { FormControlState, SliderSchema, SliderValueChangeDetails, SliderValueChangeEndDetails, SliderValueTextDetails } from '@xihan-ui/headless'
import { createCounterIdGenerator, createScope, normalizeItemIndex } from '@xihan-ui/core'
import { connectSlider, resolveFormControlState, sliderAnatomy, sliderMachine, sliderMeta } from '@xihan-ui/headless'
...
export class XhSliderElement extends XhElement {
  ...
  protected wire(): void {
    ...
    put('tick-group', api.getTickGroupProps() as Record<string, unknown>)

    // 刻度是多实例 part：身份取节点自报的 value 属性
    for (const el of this.getParts('tick'))
      this.spreader.spread(el, api.getTickProps({ value: Number(el.getAttribute('value')) }) as Record<string, unknown>)
    for (const el of this.getParts('tick-label'))
      this.spreader.spread(el, api.getTickLabelProps({ value: Number(el.getAttribute('value')) }) as Record<string, unknown>)
