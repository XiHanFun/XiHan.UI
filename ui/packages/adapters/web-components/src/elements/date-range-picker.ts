...
function declaredIndex(el: HTMLElement, position: number): number {
  ...
  return resolveDateRangePickerPanelIndex(raw == null ? undefined : raw.trim(), position)
}
...
export class XhDateRangePickerElement extends XhPortalHostElement {
  ...
  static override partContract = {
    ...
    meta: dateRangePickerMeta,
    delegates: [dateFieldAnatomy, calendarRangePickerAnatomy],
  }
  ...
  declare translations?: DateRangePickerSchema['props']['translations']
  ...
  private readonly notifyOpen = (details: DateRangePickerOpenChangeDetails): void => {
  ...
  private readonly notifyFocus = (details: DateRangePickerFocusChangeDetails): void => {
  ...
  private readonly rootCtrl = new MachineController<DateRangePickerSchema>(
    this,
  ...
  private readonly calendarCtrl = new MachineController<CalendarRangePickerSchema>(
    this,
    calendarRangePickerMachine,
    () => dateRangePickerCalendarProps(this.rootCtrl.service),
    {
  ...
  private readonly fieldCtrl = new MachineController<DateFieldSchema>(
    this,
    dateFieldMachine,
    () => dateRangePickerFieldProps(this.rootCtrl.service),
    { scope: this.pickerScope },
  )
  ...
  private readonly fieldEndCtrl = new MachineController<DateFieldSchema>(
    this,
    dateFieldMachine,
    () => dateRangePickerFieldEndProps(this.rootCtrl.service),
    { scope: this.pickerScope },
  )
  ...
  private machineProps(): Partial<DateRangePickerSchema['props']> {
  ...
  private injectRefs(svc: Service<DateRangePickerSchema>): void {
    this.ensureConfig()
  ...
  /** 两端都落定时的规范化周期值（首尾与回显键）；缺一端即 null。 */
  ...
  private wireSegments(owner: HTMLElement, field: DateRangePickerFieldApi): void {
  ...
  protected wire(): void {
    const api = connectDateRangePicker(this.services(), wcNormalize)
    ...
    for (const index of [0, 1] as const) {
      const field = dateRangePickerFieldAt(api, index)
  ...
  override disconnectedCallback(): void {
    ...
    this.exit?.dispose()
    this.exit = null
    if (this.rootCtrl.service.state.get() !== 'open')
