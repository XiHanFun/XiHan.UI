...
/**
 * `<xh-calendar-range-picker>` —— 日历范围选择器行为宿主：先落起点再落终点。
 *
 ...
 * @prop {string[]} value - 受控区间两端（升序 ISO 串）；缺省即非受控，只走 property
 ...
 * @prop {Partial<CalendarRangePickerTranslations>} translations - 读屏文案（挑区间的提示、区间两端的名字、今天），只走 property
