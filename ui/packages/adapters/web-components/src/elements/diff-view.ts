import type { DiffModel, DiffSide, DiffViewApi, DiffViewExpandedChangeDetails, DiffViewMode, DiffViewSchema, DiffViewTranslations } from '@xihan-ui/headless'
 ...
 * @csspart root - 外壳，承载 data-view / data-truncated
  ...
  #paint(api: DiffViewApi): void {
    ...
    const signature = `${api.view}|${api.rows
      .map(row => (row.kind === 'gap'
        ...
        : `l${row.rowIndex}:${row.line!.change}:${row.line!.text}`))
      .join('\n')}`
