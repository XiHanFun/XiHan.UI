...
// 验证 dialog service sections 相关行为。
...
describe('命令对话框公共三段结构', () => {
  ...
  it('函数长正文只滚 Body，Header/Footer 不随内容滚走', async () => {
    act(() => {
      service = createDialogService()
      void service.confirm({ title: '长正文', content: () => <div style={{ height: '1800px' }}>完整记录</div> })
    })
    await settle()
  ...
  it('prompt 表单位于 Body，确认仍返回业务值', async () => {
    let result: Promise<{ name: string } | null>
    act(() => {
      service = createDialogService()
      result = service.prompt({ title: '命名', initialValue: { name: '默认名称' }, body: value => <input value={value.name} readOnly /> })
    })
    await settle()
  ...
  it('告知框保留单按钮；异步确认期间公共按钮保持忙态', async () => {
    ...
    act(() => {
      service = createDialogService()
      result = service.info({ title: '处理中', onOk: () => new Promise<void>((resolve) => {
        complete = resolve
      }) })
    })
    await settle()
