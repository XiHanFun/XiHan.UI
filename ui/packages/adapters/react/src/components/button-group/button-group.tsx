import type { ActionVariant, Size, Tone } from '@xihan-ui/core'
import type { ButtonGroupProps } from '@xihan-ui/headless'
import type { ComponentPropsWithRef, ReactNode } from 'react'
import { connectButtonGroup } from '@xihan-ui/headless'
import { withXhConfig } from '../../config/config'
import { mergeReactProps } from '../../runtime/merge-props'
import { reactNormalize } from '../../runtime/normalize-props'
import { ButtonGroupApiProvider, ButtonGroupDisabledProvider, useButtonGroupApi } from './context'

export interface XhButtonGroupProps extends ComponentPropsWithRef<'div'> {
  /** 排布：horizontal / vertical，决定相邻两段在哪个轴上合边。 */
  orientation?: 'horizontal' | 'vertical'
  variant?: ActionVariant
  tone?: Tone
  size?: Size
  /** 整组禁用：组内每一段跟着禁用，段自己写了禁用的仍然禁用。 */
  disabled?: boolean
  /** 撑满行宽：整组占满可用宽度，每段等分剩余空间。 */
  fullWidth?: boolean
}

/** 一组连排的按钮：组内每一段是作者放进 children 的按钮，直接当直接子节点摆。 */
export function XhButtonGroup({
  orientation,
  variant,
  tone,
  size,
  disabled,
  fullWidth,
  children,
  ...rest
}: XhButtonGroupProps): ReactNode {
  const configured = withXhConfig('button-group', {
    orientation,
    variant,
    tone,
    size,
    disabled,
    fullWidth,
  } as ButtonGroupProps)
  const api = connectButtonGroup(configured, reactNormalize)
  return (
    <ButtonGroupApiProvider value={api}>
      {/* 组内每一段收到的是真禁用：只打 data-* 的话按钮照样点得动 */}
      <ButtonGroupDisabledProvider value={api.disabled}>
        <div {...mergeReactProps(api.getRootProps() as Record<string, unknown>, rest as Record<string, unknown>)}>
          {children}
        </div>
      </ButtonGroupDisabledProvider>
    </ButtonGroupApiProvider>
  )
}

export interface XhButtonGroupSeparatorProps extends ComponentPropsWithRef<'span'> {}

/** 段与段之间的装饰线；纯视觉，读屏不念。 */
export function XhButtonGroupSeparator({ children, ...rest }: XhButtonGroupSeparatorProps): ReactNode {
  const api = useButtonGroupApi()
  return (
    <span {...mergeReactProps(api.getSeparatorProps() as Record<string, unknown>, rest as Record<string, unknown>)}>
      {children}
    </span>
  )
}
