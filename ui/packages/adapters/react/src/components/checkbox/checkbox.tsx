...
export function XhCheckbox({
  ...
  // children 是方框旁的文字：<label> 包住两者，点文字即切换。没给文字就只有方框
  ...
  const box = (
    ...
    >
      <span {...api.getIndicatorProps() as Record<string, unknown>}>{indicator}</span>
  ...
  return (
    <label {...api.getLabelProps() as Record<string, unknown>} ref={rootRef as RefObject<HTMLLabelElement>}>
      {box}
...
XhCheckbox.xhEvents = ['checked-change'] as const
