...
const CONTROL_KEYS: Array<keyof FormControlState> = ['disabled', 'readOnly', 'required', 'invalid']
...
export function useFormControlProps<T extends FormControlState>(props: T): T {
  ...
  const state = resolveFormControlState(props, fieldState, formState)
  const resolved = { ...props }
  // 只回写调用方本来支持的轴；例如 Slider 没有 required，不能因共享 resolver 擅自长出新 prop。
  for (const key of CONTROL_KEYS) {
    if (Object.hasOwn(props, key))
      Object.assign(resolved, { [key]: state[key] })
  }
  return resolved
}
