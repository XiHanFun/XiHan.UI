...
export interface XhComboboxRootProps extends RootElementProps {
  ...
  loop?: boolean
  placeholder?: string
  /** 自动铺开时是否渲染清空按钮；手写部件模式不使用它，写了节点即可清空。 */
  clearable?: boolean
...
/** 控件盒：输入框、清空按钮与展开按钮在其中并排，描边、底色与聚焦环都落在它上面。 */
...
export interface XhComboboxInputProps extends Omit<ComponentPropsWithRef<'input'>, 'value' | 'defaultValue'> {
  /**
   * 输入框渲染为哪个标签，默认 input。
   * 写 textarea 即多行宿主：connect 随之撤销 type、role 与 aria-expanded。
   */
...
/** 放在 positioner 中作为 content 的兄弟节点，不进入 role=listbox。 */
