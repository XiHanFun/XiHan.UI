...
/**
 * `<xh-avatar>` —— Light-DOM 行为宿主：用户写 root/image/fallback 角色节点，
 * 元素跑 avatar 机器并把 connect 产出打上去。图片来源由宿主的 src/alt 写进 image 节点，
 * 加载成败经 image 自己的 load/error 回送给机器；两个节点都常挂，靠 hidden 互斥显隐，
 * 加载完成前一直是 fallback 在台前（不闪白）。
 *
 ...
 * @csspart root - 头像根容器（承载 data-status）
 * @csspart image - 图片节点，必须是原生 img；src/alt 由宿主写入（作者别自己写，会被覆盖或清掉），未就绪时带 hidden
 * @csspart fallback - 图片之外的回退内容（首字母、图标），图片就绪后带 hidden
 */
export class XhAvatarElement extends XhElement {
  ...
  // avatar 机器不碰 DOM、不用定位与消解层：不需要 config/layer/refs/scope，controller 只带 props。
