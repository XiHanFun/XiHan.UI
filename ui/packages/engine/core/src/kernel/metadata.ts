...
//
// 版本从 package.json 派生(库包锁步同版,core 的版本就是整套库的版本,改版本只改
...
/** 框架版本:从 core 的 package.json 派生,库包锁步同版下即整套库的版本。 */
...
/**
 * 曦寒标志。给终端启动横幅用,见 @xihan-ui/core/vite。
 *
 ...
 * 类型对不上甚至运行时挂掉。适配器 dev 启动时拿自身版本与 core 比对,不一致经
 * 诊断通道发一条 warn;版本信息归元数据管,所以与 Framework 一样住在这个模块里。
 */
export function checkLockstepVersion(name: string, version: string, coreVersion: string): void {
  if (version === coreVersion)
    return
  reportDiagnostic({
    code: DIAGNOSTIC_CODES.versionMismatch,
    level: 'warn',
    message: `${name} ${version} 与 core ${coreVersion} 版本不一致——全部库包锁步发版,混装会类型对不上甚至运行时挂掉,请统一到同一版本`,
    detail: { name, version, coreVersion },
  })
}
 ...
 * 启动横幅归开发者的终端——那一份由 @xihan-ui/core/vite 打,与 Framework 侧
