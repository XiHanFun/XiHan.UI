...
describe('collapsibleMachine 按压通道：Space / Enter 与触屏按住投影 data-pressed', () => {
  it('keydown 在场、keyup 撤下；触屏按下在场、抬起 / 取消撤下；失焦撤下；鼠标按下不走这一路；开合不动', () => {
    const c = makeCollapsible()
    expect(c.trigger()['data-pressed']).toBeUndefined()
    fire(c.trigger(), 'onKeyDown', key(' '))
    expect(c.trigger()['data-pressed']).toBe('')
    fire(c.trigger(), 'onKeyUp', key(' '))
    expect(c.trigger()['data-pressed']).toBeUndefined()
    fire(c.trigger(), 'onKeyDown', key('Enter'))
    expect(c.trigger()['data-pressed']).toBe('')
    fire(c.trigger(), 'onBlur', {})
    expect(c.trigger()['data-pressed']).toBeUndefined()
    fire(c.trigger(), 'onPointerDown', { pointerType: 'touch' })
    expect(c.trigger()['data-pressed']).toBe('')
    fire(c.trigger(), 'onPointerCancel', {})
    expect(c.trigger()['data-pressed']).toBeUndefined()
    fire(c.trigger(), 'onPointerDown', { pointerType: 'touch' })
    expect(c.trigger()['data-pressed']).toBe('')
    fire(c.trigger(), 'onPointerUp', {})
    expect(c.trigger()['data-pressed']).toBeUndefined()
    fire(c.trigger(), 'onPointerDown', { pointerType: 'mouse' })
    expect(c.trigger()['data-pressed']).toBeUndefined()
    expect(c.state()).toBe('closed')
    expect(c.changes).toEqual([])
    c.stop()
  ...
  it('按住途中翻面：Enter 在 keydown 即切换开合，按压面不随之丢，keyup 才撤下；两个状态都接按压', () => {
    const c = makeCollapsible()
    fire(c.trigger(), 'onKeyDown', key('Enter'))
    expect(c.trigger()['data-pressed']).toBe('')
    c.click()
    expect(c.state()).toBe('open')
    expect(c.trigger()['data-pressed']).toBe('')
    fire(c.trigger(), 'onKeyUp', key('Enter'))
    expect(c.trigger()['data-pressed']).toBeUndefined()
    fire(c.trigger(), 'onPointerDown', { pointerType: 'touch' })
    expect(c.trigger()['data-pressed']).toBe('')
    c.click()
  ...
  it('disabled：按住不进；经 signal 转禁用时按住的 trigger 自收', () => {
    const off = makeCollapsible({ disabled: true })
