...
describe('connectTreeSelect 家族角色', () => {
  it('control 投影 Field Chrome 的稳定角色与尺寸档（缺省 md），三个状态属性都在盒上', () => {
    const control = mount({ readOnly: true, invalid: true }).api().getControlProps() as Record<string, unknown>
    ...
    expect((mount({ disabled: true }).api().getControlProps() as Record<string, unknown>)['data-disabled']).toBe('')
    expect((mount({ size: 'lg' }).api().getControlProps() as Record<string, unknown>)['data-xh-field-size']).toBe('lg')
  })
  ...
  it('清空钮走 Action Control 的 field-inset ghost 档，按 has-value 显隐，尺寸档随 size 缺省 md', () => {
    ...
    expect(mount({ defaultValue: 'license' }).clear.getAttribute('data-xh-action-has-value')).toBe('')
    expect(mount({ size: 'sm' }).clear.getAttribute('data-xh-action-size')).toBe('sm')
  })

  it('叶子行与分支行都投影 Collection Item 的 overlay 语境与尺寸档；正文、对号与展开箭头各落自己的槽', () => {
    const h = mount({ defaultValue: 'license' })
    const leaf = h.item('license')
    const branch = h.branch('src')
    for (const row of [leaf.item, branch.control]) {
      expect(row.getAttribute('data-xh-collection-item')).toBe('')
      expect(row.getAttribute('data-xh-collection-size')).toBe('md')
      expect(row.getAttribute('data-xh-collection-context')).toBe('overlay')
    }
    expect(leaf.text.getAttribute('data-xh-collection-slot')).toBe('text')
    expect(leaf.indicator.getAttribute('data-xh-collection-slot')).toBe('indicator')
    expect(branch.text.getAttribute('data-xh-collection-slot')).toBe('text')
    expect(branch.trigger.getAttribute('data-xh-collection-slot')).toBe('prefix')
    expect(branch.indicator.getAttribute('data-xh-collection-slot')).toBe('prefix')
    // 分支行本身不是 treeitem：aria-selected 在 branch 上，行上只有 data-selected 供皮肤显示对号
    expect(branch.control.hasAttribute('aria-selected')).toBe(false)
    expect(mount({ size: 'lg' }).item('license').item.getAttribute('data-xh-collection-size')).toBe('lg')
  })
})
