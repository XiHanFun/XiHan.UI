...
describe('connectDatePicker 家族角色', () => {
  ...
  it('日历钮常驻、清空钮按 has-value 显隐，都走 field-inset ghost 档，尺寸档随 size 缺省 md', () => {
    ...
    expect(mount({ defaultValue: ['2026-07-10'] }).clear.getAttribute('data-xh-action-has-value')).toBe('')
  ...
  it('快捷选项与时间格都投影 Collection Item 的 overlay 语境与尺寸档', () => {
    ...
    const time = h.api().getTimeItemProps({ unit: 'hour', value: '08' }) as Record<string, unknown>
    for (const row of [preset, time]) {
    ...
    expect((mount({ size: 'lg', presets: [{ value: '2026-07-10', label: '某日' }] }).api().getPresetProps({ value: '2026-07-10' }) as Record<string, unknown>)['data-xh-collection-size']).toBe('lg')
  })
})
