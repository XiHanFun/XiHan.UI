...
import type { MatrixCodeEyeShape, MatrixCodeLogoArea, MatrixCodeModuleShape, MatrixCodeProps, QrLevel } from '../src/matrix-code'
import {
  DIAGNOSTIC_CODES,
  normalizeProps,
  onDiagnostic,
...
import { connectMatrixCode, qrAlignmentPositions, qrCapacityBytes, qrEncode } from '../src/matrix-code'

/** logo 损伤警告的诊断码。写成字面量，订阅方按它分流，改了就是改契约。 */
const LOGO_DAMAGE_CODE = 'matrix-code.logo-damage'
...
interface Painted {
  ...
  layers: Layer[]
  /** 盖在两条 path 之上的挖空矩形。 */
  clear: MatrixCodeLogoArea | undefined
}
...
function paintedOf(pathD: string, eyeD: string, clear: MatrixCodeLogoArea | undefined): Painted {
  return { layers: [makeLayer(parsePath(pathD)), makeLayer(parsePath(eyeD))], clear }
}

interface Readback {
  api: ReturnType<typeof connectMatrixCode>
...
const MODULE_SHAPES: readonly MatrixCodeModuleShape[] = ['square', 'dot', 'rounded']
const EYE_SHAPES: readonly MatrixCodeEyeShape[] = ['square', 'rounded']
const LEVELS: readonly QrLevel[] = ['L', 'M', 'Q', 'H']

/** 六种形状组合。逐格读回墨的那几条判据一条用例只跑一组，工作量不叠在一起。 */
const SHAPE_COMBOS: readonly (readonly [MatrixCodeModuleShape, MatrixCodeEyeShape])[] = MODULE_SHAPES
...
/** 逐格对账，最多列出前 8 处不同。 */
function mismatches(ink: readonly boolean[][], matrix: readonly (readonly boolean[])[], clear?: MatrixCodeLogoArea, margin = 0): string[] {
  const out: string[] = []
...
describe('缺省形状的几何总量不变', () => {
  ...
  it.each([0, 1, 4, 9])('margin %d：square + square 的 path + eyePath 与整张码合并出来的那条盖住同一片墨', (margin) => {
    for (const { value, level } of SAMPLES) {
      const api = connectMatrixCode({ value, level, margin }, normalizeProps)
  ...
  it('显式写死缺省值、以及只写其中一个，几何逐字不变', () => {
    const base = connectMatrixCode({ value: '曦寒 UI', level: 'M' }, normalizeProps)
    for (const props of [
      ...
      const api = connectMatrixCode({ value: '曦寒 UI', level: 'M', ...props }, normalizeProps)
  ...
  it('六种形状组合都拆出 eyePath，且 path 里不含码眼那三块', () => {
    // 码眼恒独立成一条，皮肤的 --xh-matrix-code-eye-fg 才有落地的节点；缺省形状也不例外
    for (const moduleShape of MODULE_SHAPES) {
      for (const eyeShape of EYE_SHAPES) {
        const api = connectMatrixCode({ value: '曦寒 UI', level: 'M', moduleShape, eyeShape }, normalizeProps)
  ...
  it('形状给了词表外的值时退到盖得住格心的画法，扫得出的码不变成扫不出的', () => {
    ...
    for (const [moduleShape, eyeShape] of [['circle', 'square'], ['square', 'circle'], ['circle', 'circle']] as const) {
      const got = readback({
        ...
        level: 'M',
        moduleShape: moduleShape as MatrixCodeModuleShape,
        eyeShape: eyeShape as MatrixCodeEyeShape,
      })
      expect({ moduleShape, eyeShape, bad: mismatches(got.ink, got.matrix) })
        .toEqual({ moduleShape, eyeShape, bad: [] })
    }
...
describe('时序图形与校正图形没被变形', () => {
  ...
  it.each(MODULE_SHAPES)('%s 码点下，深色的时序 / 校正模块整格都在某一个矩形子路径里', (moduleShape) => {
    for (const { value, level } of SAMPLES) {
      const api = connectMatrixCode({ value, level, moduleShape, eyeShape: 'rounded' }, normalizeProps)
      const matrix = qrEncode(value, level).modules
...
describe('logo 挖空', () => {
  it('挖空区整格居中、边长是奇数、不超过每边模块数的 1/5', () => {
    for (const { value, level } of SAMPLES) {
      for (const margin of [0, 4, 9]) {
        const api = connectMatrixCode({ value, level, margin, logo: true }, normalizeProps)
  ...
  it('挖空区一格都不碰定位图形、时序图形、格式信息与版本信息', () => {
    ...
    for (const version of [1, 2, 6, 7, 20, 32, 40] as const) {
      ...
      const api = connectMatrixCode({ value, level: 'H', logo: true }, normalizeProps)
  ...
  it('挖掉的码字比例在 Q / H 的纠错余量之内，L / M 则赔不起', () => {
    ...
    for (const version of [1, 2, 6, 7, 14, 20, 27, 32, 40] as const) {
      ...
      for (const level of LEVELS) {
        ...
        const api = connectMatrixCode({ value, level, logo: true }, normalizeProps)
        expect(api.version).toBe(version)
        const area = api.logoArea!
  ...
  it('没画出码时不留位：内容超容量落到 error 也一样', () => {
    const api = connectMatrixCode({ value: 'x'.repeat(3000), level: 'H', logo: true }, normalizeProps)
...
describe('logo 损伤量与警告', () => {
  it('没留 logo 位就没有损伤可报', () => {
    for (const props of [
      ...
      expect(connectMatrixCode(props, normalizeProps).logoDamage).toBeUndefined()
    }
  ...
  it('2 版那张峰值码：25 个模块、44 个码字里的 6 个', () => {
    ...
    const api = connectMatrixCode({ value, level: 'L', logo: true }, normalizeProps)
  ...
  it('警告说清了怎么办：把 level 提到 Q 或 H', () => {
    const value = 'x'.repeat(qrCapacityBytes(2, 'L'))
    connectMatrixCode({ value, level: 'L', logo: true }, normalizeProps)
    const [record] = damageWarnings()
    expect(record).toBeDefined()
    expect(DIAGNOSTIC_CODES.matrixCodeLogoDamage).toBe(LOGO_DAMAGE_CODE)
    expect({ code: record!.code, level: record!.level, scope: record!.scope })
      .toEqual({ code: LOGO_DAMAGE_CODE, level: 'warn', scope: 'matrix-code' })
...
describe('判据自身有牙', () => {
  ...
  it('抠掉 path 里一段子路径，中心对账立刻报出缺墨', () => {
    const api = connectMatrixCode({ value: '曦寒 UI', level: 'M', moduleShape: 'dot' }, normalizeProps)
