...
const api = (service: Service<DialogSchema>) => connectDialog(service, normalizeProps)
...
describe('connectDialog：两颗按钮接 Action Control', () => {
...
describe('dialogMachine 按压通道：Space / Enter 与触屏按住投影 data-pressed，按住的是哪颗就只落在哪颗上', () => {
  it('触发器：keydown 在场、keyup 撤下；触屏按下在场、抬起撤下；失焦撤下；鼠标按下不走这一路', () => {
  ...
  it('面板收起即松开：按住 Enter 关掉对话框，关闭钮不会再来 keyup，按压面由机器收', () => {
    const service = makeService({ defaultOpen: true })
    const close = (): Dict => api(service).getCloseTriggerProps() as Dict
