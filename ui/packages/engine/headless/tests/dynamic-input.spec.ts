...
import type { DynamicInputSchema } from '../src/dynamic-input'
import { normalizeProps } from '@xihan-ui/kernel'
import { createService } from '@xihan-ui/machine'
import { createVanillaRuntime } from '@xihan-ui/machine/vanilla'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { connectDynamicInput, dynamicInputMachine, dynamicInputTriggerId } from '../src/dynamic-input'
import { atRowMax, atRowMin, moveRow, rowBound, sameRows } from '../src/dynamic-input/dynamic-input.machine'

type Props = DynamicInputSchema['props']

function makeService(props: Props = {}) {
  const runtime = createVanillaRuntime()
  // 同一个对象原样返回：service 按返回值身份缓存 props，原地改字段即可模拟宿主写回
  const service = createService(dynamicInputMachine, { props: () => props, runtime })
  runtime.start()
  return service
}
...
function api(service: Svc) {
  return connectDynamicInput(service, normalizeProps)
}
