...
const api = (service: Service<DrawerSchema>) => connectDrawer(service, normalizeProps)
...
describe('connectDrawer：两颗按钮接 Action Control', () => {
...
describe('drawerMachine 按压通道：与对话框共用一台机器，按住的是哪颗就只落在哪颗上', () => {
  ...
  it('关闭钮：只有按住的那颗带 data-pressed；抽屉收起即松开，重开后关闭钮是干净的', () => {
    const service = makeService({ defaultOpen: true })
    const trigger = (): Dict => api(service).getTriggerProps() as Dict
