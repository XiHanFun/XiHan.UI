...
function hover(el: HTMLElement, pointerType = 'mouse'): void {
  el.dispatchEvent(new PointerEvent('pointerenter', { bubbles: false, pointerType }))
}

/** 有接触面的真实按下：宽高给足，免得被当成读屏合成的那一下。 */
function pointerDown(el: HTMLElement, pointerType = 'mouse'): void {
...
function pointerUp(el: HTMLElement, pointerType = 'mouse'): void {
...
describe('区间模式', () => {
  ...
  it('挑到一半时旧区间不亮：亮的是起点到悬停那一段', () => {
    const h = mount({ defaultFocusedValue: '2024-02-15', selectionMode: 'range', defaultValue: ['2024-02-03', '2024-02-05'] })
  ...
  it('落起点后后一格挑不了就退到前一格', async () => {
    const h = mount({ defaultFocusedValue: '2024-02-10', selectionMode: 'range', max: '2024-02-10' })
    h.cell('2024-02-10').focus()
    press(h.cell('2024-02-10'), 'Enter')
    await settle()
  ...
  it('按在已选区间的一端上拖：改写那一端；原地松开则从那一端重新开始', () => {
    const h = mount({ defaultFocusedValue: '2024-02-15', selectionMode: 'range', defaultValue: ['2024-02-10', '2024-02-14'] })
    // 拖终点到 16 号
    pointerDown(h.cell('2024-02-14'))
    expect(h.api().rangeAnchor).toBe('2024-02-10')
  ...
  it('受控值被宿主回写：起点同样作废', () => {
    ...
    const h = mountDrill({ defaultFocusedValue: '2024-02-15', selectionMode: 'range', value: [] })
    click(h.cell('2024-02-10'))
    expect(h.api().rangeAnchor).toBe('2024-02-10')
  ...
  it('作者标了 invalid 也走同一条：根与区间里的格子都报', () => {
    const h = mount({ selectionMode: 'range', defaultValue: ['2024-02-10', '2024-02-12'], invalid: true })
    expect(h.root.getAttribute('data-invalid')).toBe('')
    expect(h.cell('2024-02-10').getAttribute('aria-invalid')).toBe('true')
  })

  it('聚焦格上提示这一下是开始还是收尾；区间两端的可及名字带上整段起止', () => {
    const h = mount({ defaultFocusedValue: '2024-02-10', selectionMode: 'range', locale: 'en-US' })
  ...
  it('只读：起点落不下，预览也不铺', () => {
    const h = mount({ defaultFocusedValue: '2024-02-15', selectionMode: 'range', readOnly: true })
    pointerDown(h.cell('2024-02-10'))
    pointerUp(h.cell('2024-02-10'))
    click(h.cell('2024-02-10'))
    expect(h.api().rangeAnchor).toBeNull()
...
describe('标题钻取', () => {
  ...
  it('选择模式独立切换，并按新模式收口现值', () => {
    ...
    // 切回区间：留下的那一个值不续作起点，再挑仍是先起点后终点
    h.setProps({ selectionMode: 'range' })
    h.api().select('2026-03-18')
    expect(h.value()).toEqual(['2026-02-18'])
    h.api().select('2026-03-20')
    expect(h.value()).toEqual(['2026-03-18', '2026-03-20'])
  })
