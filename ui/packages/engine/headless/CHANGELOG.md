...
- 4a82bc7: 带子项的组件，条目契约本身支持逐条语气：数据里写一个 `tone`，这一条就按该族颜色表达。
...
- cd2be1f: `MenuNode` 新增 `tone`：菜单里"移到回收站"这一类命令可以逐条声明自己的性质。
...
- 19570ad: **dialog 不再发 `data-position`。** 这个属性恒为 `'center'`：没有入参改得动它，取值集合只有一个元素，自带皮肤里没有一条规则读它，它对任何人都不产生任何信息。半成品不留在公开面上等，删掉。
...
- 294dfa4: 修复表单并发校验的忙碌状态，由当前有效异步任务统一派生。
...
- f30630d: Layout 的 `siderBreakpoint` 现在跟随运行期属性变化切换媒体查询，移除档位时解除观察并清除窄屏状态。
...
- 45e4ab9: ColorSwatchPicker 的 `label` 部件新增投影 `data-disabled`：整组 `disabled` 时标签随之置灰（皮肤的 `--xh-color-swatch-picker-label-fg-disabled` 从此有了真实的消费状态）；单格禁用只是那一格的事，不写到标签上。三端与一致性套件同步断言。
...
- 7b187ca: **收敛表单一族的错误播报**：一次校验失败只留一处打断式活区。
...
- 0dd39b5: **修复** `calendar-picker` 与 `calendar-range-picker`：指针按在邻月的日子上时，浏览器把焦点落上去那一下不再翻页。
...
- abcb2c1: 修复日历选择器与日历范围选择器点击邻月日期时，聚焦通知早于视窗翻页导致作者渲染的月份、点击日期与内部状态错位的问题。邻月格在按压期间仅保留临时焦点，确认点击或区间终点后再提交聚焦值并通知重画，正向与反向选择都会落在实际点击的日期。
- 785be0a: **日历的周视图：点或走到某一页的首尾周时视窗不再翻到邻月。**
...
- 17e3d21: **修复** `cascader` 集合为空（首次取数、无数据）时展开的初始焦点。此前焦点域先去落根列，而皮肤在空态下把根列 `display:none` 让位给占位面，浏览器不让它接住 `focus()`，焦点要等焦点域逐帧重试到最后一帧才兜底落到 content 上。现在与连接层「集合为空时由 content 兜底进 Tab 序列」同一口径，初始焦点直接落到 content。
- 0daae33: **`cascader` 的检索档补上键盘规格，并修掉规格照出来的六处缺陷。**
...
- 1b713d6: 修正 Cascader 搜索结果与列项的选择状态分叉。
...
- b483c3b: Command 按 WAI-ARIA combobox 的 selection-follows-focus 模式同步活动候选：
...
- 9a7827b: **修复** `date-picker`：展开时没有选中值先落 `defaultFocusedValue` 那一页，再没有才落到今天；此前 `defaultOpen` 与重新展开都会把作者给的起始页覆盖成今天。
...
- 651befe: Cascader 空数据/首次加载不再要求 column 或 item，TreeSelect 空树或只有分支时不再要求 item。
...
- 67a4aeb: **`floating-panel` 的内建默认矩形现在会夹进视口。** 此前 360×240 落在 (24,24)，右缘恒在 384：375 宽的视口上面板连同右侧那三个改尺把手一起落在屏外，指针够不着、方向键也看不见。
...
- 4fe1897: 修复表单正则规则复用带 g/y 标志的 RegExp 时，同一输入交替通过和失败的问题。
  每次校验独立匹配，不读取或修改调用者的 lastIndex；同步首败即停和异步规则顺序保持一致。
- 548ef58: 复用 Headless 内部的数组逐位比较、单值归一与保序去重实现，保持受控值、空值和集合顺序语义不变，同时收回主入口重复运行时代码。
- 8e6070d: 复用浮层连接层的固定坐标、可用空间、锚点宽度与箭头坐标构造器。各组件既有的
...
- de42a74: **修 `image-viewer` 的退场动画在三家适配器上一帧都播不出来。**
...
- 0daae33: **`image-viewer` 底部那条控件带改报 `role="group"`，不再报 `role="toolbar"`。**
...
- 0e5bed0: **修复** `kbd` 根元素的可读名称对读屏不可见。原生 `kbd` 对应 generic 角色，ARIA 禁止在它身上写 `aria-label`（axe 报 `aria-prohibited-attr`），读屏会直接忽略；视觉键帽各自 `aria-hidden`，整组于是一个名字都没有。根元素现在投影 `role="img"`，由它承接 `aria-label`：放进按钮或菜单项时名字照常汇入宿主的可访问名称。
...
- c1ddd60: 所有浮层机器的 layer effect 现在通过统一的初始化事务登记 LayerRegistry、DismissableLayer、FocusScope、ScrollLock 与背景失活资源。同步步骤或宿主 flush 中延后执行的背景失活初始化失败时，已经取得的 layer 资源都会逆序完整回滚，不再留下占据共享栈顶但没有 effect cleanup 的半初始化 layer。
...
- 2962b7d: Select 在初始异步加载或无选项时允许 `list` 内没有 `item`。移除静态元数据中错误的必需选项要求，
...
- 7cf8734: **时间范围选择器在区间编辑期间保持时列结构稳定。** `min`、`max` 与另一端形成的边界不再删除小时、分钟、秒和上下午选项，而是将界外项标为禁用，避免列高、滚动位置和焦点节点随填写进度跳变。
...
- afeb764: 修复 Truncate 在网页字体加载后不重新测量的问题，并确保尺寸、正文与字体观察器在组件卸载时全部清理。
...
- 1b03cdb: `carousel` 的划动收进多指会话：`carousel.connect.ts` 里的 `setPointerCapture` 与 `onPointerMove` / `onPointerUp` / `onPointerCancel` 三个处理器一并撤掉，连接层只报落点。至此 M0 记的两处指针捕获遗留清空。
...
- 5b62d15: **新增** 选中原语 `applySelection` / `toggleSelectAll` / `rangeBetween`：带锚点的范围选与全选，纯集合运算，不碰 DOM。
...
- 9ae6c64: **改动** `tree` 的拖拽播报报出落脚处。树的搬家是换父，位次说不清去向：把「甲」拖进「收件箱」
...
- be46eab: **修复** 从拖动把手起手之后，焦点停在把手上而不是它所属的那一行 / 那个节点。把手是
...
- df08c58: **修复** 按住切换类的键不放时会来回翻转：按住 <kbd>Ctrl</kbd>/<kbd>Cmd</kbd> + <kbd>A</kbd> 会在全选与全不选之间一直闪，按住空格会反复拾起放下。
