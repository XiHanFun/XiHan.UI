...
export interface MarqueeProps {
  ...
  /**
   * 名义上的每秒像素数。写成根上的内联变量，皮肤拿一份内容的长度除以它换成一圈的时长。
   * 那个长度取的是 `--xh-marquee-span`——CSS 读不到布局尺寸，槽里放的是一个缺省值。
   * 把它改到与内容真实长度一致时速度才逐字等于每秒这么多像素，否则它是一个成比例的快慢档。
   * 只收有限正数；其余值不写出，退回皮肤缺省。
   */
  speed?: number
  /** 指针停在窗口上时暂停；键盘焦点落进窗口时同样暂停。 */
  pauseOnHover?: boolean
  /** 受控暂停：翻真即停在当前位置，翻假接着走。比 pauseOnHover 优先。 */
  paused?: boolean
