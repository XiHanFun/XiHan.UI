...
export const tagGroupKeyboard: KeyboardTable = {
  ...
  source: APG,
  rows: [
    { id: 'tag-group.kbd.tab', keys: ['Tab', 'Shift+Tab'], when: 'focus outside the group', does: '整组只占一个 Tab 位：焦点进入锚点标签，无锚点时先落到列表容器再由它转移；每个标签的移除按钮一律不占停靠点' },
    { id: 'tag-group.kbd.next', keys: ['ArrowRight'], when: 'focus in group, orientation=horizontal', does: '焦点移到下一个可停留标签（禁用项跳过、尽头按 loop 回绕）；orientation=vertical 时改由 ArrowDown 承担，dir=rtl 再对调左右' },
    { id: 'tag-group.kbd.prev', keys: ['ArrowLeft'], when: 'focus in group, orientation=horizontal', does: '焦点移到上一个可停留标签（禁用项跳过、尽头按 loop 回绕）；orientation=vertical 时改由 ArrowUp 承担，dir=rtl 再对调左右' },
    ...
    { id: 'tag-group.kbd.delete', keys: ['Delete', 'Backspace'], when: 'focus on item, 该标签可移除且可修改', does: '移除焦点标签，并把焦点交给前一个；前面没有则交给后一个，全部移除后交给列表容器' },
