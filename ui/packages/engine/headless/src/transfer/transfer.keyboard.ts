...
export const transferKeyboard: KeyboardTable = {
  ...
  source: APG,
  rows: [
    { id: 'transfer.kbd.press', keys: ['Enter', 'Space'], when: 'held on 条目 / 全选格 / 搬运按钮, 未禁用、未只读、未加载且部件自身可用', does: '按住期间条目、全选格或搬运按钮投影 data-pressed，与指针 :active 同一副按压面；抬起或失焦撤下，搬完后按钮失去可搬的条目时由机器撤下。勾选与搬运语义照旧由这一次按键承担' },
    ...
    { id: 'transfer.kbd.extend', keys: ['Shift+ArrowDown', 'Shift+ArrowUp'], when: 'focus in a list, 本侧可勾选', does: '焦点移到相邻条目并切换它的勾选态；反向移动即取消刚扩展进来的条目' },
    { id: 'transfer.kbd.select-all', keys: ['Ctrl+A', 'Cmd+A'], when: 'focus in a list, 本侧可勾选', does: '勾中本侧全部可操作条目（可见且未禁用）；已经全勾则一并取消' },
    { id: 'transfer.kbd.move', keys: ['ArrowRight', 'ArrowLeft'], when: 'focus in a list, 该方向指向对面且可以移动', does: '把本侧勾选的条目移动到对面（dir=rtl 时左右语义对调）；移动完成后焦点落到目标侧的列表上。方向指向本侧、或当前无法移动时该键放行给页面' },
