...
export const cascaderKeyboard: KeyboardTable = {
  ...
  rows: [
    ...
    { id: 'cascader.kbd.press', keys: ['Enter', 'Space'], when: 'held in item / clear-trigger, 未禁用、未只读、未加载', does: '按住期间该部件投影 data-pressed，与指针 :active 同一副按压面；抬起或失焦撤下，条目随浮层收起一并撤下；没有值可清时清空按钮不进' },
    ...
    { id: 'cascader.kbd.search.press', keys: ['Enter'], when: 'open, focus in input, 有高亮候选且未禁用、未只读、未加载，按住', does: '按住期间高亮候选投影 data-pressed，与指针 :active 同一副按压面；抬起或失焦撤下，候选随浮层收起一并撤下' },
