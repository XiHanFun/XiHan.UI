...
export const treeSelectKeyboard: KeyboardTable = {
  ...
  rows: [
    ...
    { id: 'tree-select.kbd.select', keys: ['Enter', 'Space'], when: 'open, 焦点节点未禁用且不是错误分支', does: '选中焦点节点：单选替换并收起浮层、焦点归还 trigger；多选切换且浮层不收起', restoresFocus: true },
    { id: 'tree-select.kbd.press', keys: ['Enter', 'Space'], when: 'held in item / branch / clear-trigger, 未禁用、未只读、未加载', does: '按住期间叶子行、分支行（branch-control）或清空按钮投影 data-pressed，与指针 :active 同一副按压面；抬起或失焦撤下，节点随浮层收起一并撤下；没有值可清时清空按钮不进' },
    { id: 'tree-select.kbd.retry', keys: ['Enter', 'Space'], when: 'open, focus on branch, 分支加载失败', does: '重试该分支，焦点留在分支行；不改变选中值与展开集合' },
