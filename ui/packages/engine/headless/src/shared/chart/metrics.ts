 ...
 * 字号取 `--xh-text-caption-size`、行高取 `--xh-leading-tight`）。服务端与读不到计算样式时用它；
...
type LengthKey = 'barMax' | 'gap' | 'lineWidth' | 'pointSize' | 'hitMin' | 'tickLength' | 'labelGap'

const LENGTH_KEYS: readonly LengthKey[] = ['barMax', 'gap', 'lineWidth', 'pointSize', 'hitMin', 'tickLength', 'labelGap']
