...
export const timePickerKeyboard: KeyboardTable = {
  ...
  rows: [
    ...
    { id: 'time-picker.kbd.option-next', keys: ['ArrowDown'], when: 'open, focus in 某一列', does: '列内下移一格，到尾回绕；被 min/max 裁掉的格自动跳过' },
    { id: 'time-picker.kbd.option-prev', keys: ['ArrowUp'], when: 'open, focus in 某一列', does: '列内上移一格，到头回绕；被 min/max 裁掉的格自动跳过' },
    { id: 'time-picker.kbd.option-first', keys: ['Home'], when: 'open, focus in 某一列', does: '焦点移到本列首格' },
    { id: 'time-picker.kbd.option-last', keys: ['End'], when: 'open, focus in 某一列', does: '焦点移到本列末格' },
