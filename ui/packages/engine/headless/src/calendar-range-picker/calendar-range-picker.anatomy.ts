...
// data-part 直接用 kebab-case，与 CSS 选择器一致。部件表与日历选择器完全相同，只有 scope 不同；
// 类型标注 Anatomy<CalendarPart> 保证两边不漂移。
export const calendarRangePickerAnatomy: Anatomy<CalendarPart> = createAnatomy('calendar-range-picker', [
  'root',
  'header',
  'prev-year-trigger',
  'prev-trigger',
...
export const calendarRangePickerCellTriggerQuery: ItemQuery = { scope: calendarRangePickerAnatomy.name, part: 'cell-trigger' }
