...
export interface EllipsisExpandedChangeDetails {
  /** 此刻是不是铺开了全文。 */
  expanded: boolean
}

export interface EllipsisOverflowChangeDetails {
  /** 夹住的那一版有没有被裁掉内容。 */
  overflowing: boolean
}

/** 判定溢出要的四个实测量，单位 px。 */
export interface EllipsisMetrics {
  /** 内容在行内轴上的完整长度。 */
  scrollWidth: number
  /** 盒子在行内轴上看得见的那一段。 */
  clientWidth: number
...
/** 适配器在挂载前填入的 DOM 取值口。 */
export interface EllipsisRefs {
  /** 夹字的那个盒子：溢出与文字都量它。 */
  getRootEl: () => HTMLElement | null
}

export interface EllipsisSchema extends MachineSchema {
  props: {
    /** 夹几行，1 为单行，默认 1。 */
    lines?: number
    /** 点一下铺开全文。 */
    ...
    onExpandedChange?: (details: EllipsisExpandedChangeDetails) => void
    /** 量出来的溢出结论翻面时回调。 */
    onOverflowChange?: (details: EllipsisOverflowChangeDetails) => void
  }
  context: {
...
export interface EllipsisApi<T extends PropTypes = PropTypes> {
  /** 此刻是不是铺开了全文。 */
  expanded: boolean
