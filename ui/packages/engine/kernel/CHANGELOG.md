...
- 03fb633: **新增** `@xihan-ui/kernel/vite` 子入口：启动横幅打在开发者跑项目的那个终端里，
...
- 元数据的宿主行不再报锁步一致性，只报宿主与版本。

  `getMetadataSummary()` / `getMetadataDetails()` 里的宿主行此前会把宿主版本与 kernel 版本比一次，不同就写「锁步不一致（x ≠ y）」。这件事运行期已经有专门的一条路径在做——`registerRuntimeHost` 会发 `core.version-mismatch` 诊断，dev 下直接打到控制台；启动横幅上再报一遍，同一件事说两遍，而横幅那一遍还不带诊断码。

  现在宿主行就是 `宿主:vue v1.0.0`。锁步检查本身一点没动。
