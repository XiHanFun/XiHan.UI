...
import type { EllipsisMetrics, EllipsisSchema } from './ellipsis.types'
import { setup } from '@xihan-ui/machine'

const { createMachine } = setup<EllipsisSchema>()

/** 不给行数时夹一行。 */
export const ELLIPSIS_DEFAULT_LINES = 1

/** 压线判定的容差（px）。滚动尺寸与可视尺寸各自取整，恰好放得下的一行会差出 1 来。 */
const EDGE_TOLERANCE = 1

/** 行数归一：向下取整、至少 1；给不出有限数就退回缺省。 */
export function resolveEllipsisLines(lines: number | undefined): number {
  if (lines == null || !Number.isFinite(lines))
    return ELLIPSIS_DEFAULT_LINES
  return Math.max(1, Math.floor(lines))
}

/** 夹住的这一版有没有被裁掉内容：单行比行内轴，多行比块轴。 */
export function isEllipsisOverflowing(metrics: EllipsisMetrics, multiline: boolean): boolean {
  return multiline
...
/** 量测用得上的那几件。 */
type MeasureParams = Pick<Params<EllipsisSchema>, 'refs' | 'prop' | 'context' | 'state'>
...
function runMeasure(p: MeasureParams): void {
  ...
  const next = isEllipsisOverflowing(el, resolveEllipsisLines(p.prop('lines')) > 1)
 ...
 */
export const ellipsisMachine = createMachine({
  name: 'ellipsis',
  context: ({ cell }) => ({
