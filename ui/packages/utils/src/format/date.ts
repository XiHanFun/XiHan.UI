 ...
 * 格式化日期
 */
export const format = (date: DateValue, format = "YYYY-MM-DD HH:mm:ss"): string => {
 ...
 * 相对时间
 */
export const relative = (date: DateValue): string => {
  const now = Date.now();
 ...
 * 获取日期范围
 */
export const getRange = (date: DateValue, unit: "year" | "month" | "week" | "day"): [Date, Date] => {
  const d = new Date(date);
  let start: Date;
  let end: Date;
