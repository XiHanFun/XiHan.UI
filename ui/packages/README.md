...
packages/
  ...
  engine/     kernel  machine  motion  behavior  position  code-highlight  headless   ← 你不用管
```
...
**`engine` 不是「底层的东西」。** 它是「使用者做不了取舍的东西」。`headless` 的 121 个
...
Radix 2025-01 补发单包消化细拆后遗症。121 个组件共享同一套机器，拆了每个适配器都得
