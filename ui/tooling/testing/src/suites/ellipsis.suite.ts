...
import { ellipsisAnatomy, ellipsisKeyboard } from '@xihan-ui/headless'
...
function partEl(doc: Document, part: string): HTMLElement {
  const el = doc.querySelector<HTMLElement>(`[data-scope="ellipsis"][data-part="${part}"]`)
  if (!el)
...
export const ellipsisSuite: ConformanceSuite = {
  component: 'ellipsis',
  anatomy: ellipsisAnatomy,
  keyboard: ellipsisKeyboard,
  fixture: FIXTURE,
  cases: [
    {
