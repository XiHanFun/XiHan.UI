...
import { matrixCodeAnatomy, matrixCodeKeyboard } from '@xihan-ui/headless'

// 二维码是一张图，APG 里对应的是"命名与描述"那一节；判据锁六件：
// 命名两态互斥、版本与静区如实落到根上、没有可编码的内容时一个模块都不铺、
// 码眼恒独立成一条 path（皮肤的 --xh-matrix-code-eye-fg 要有落地的节点）、
...
function rootEl(doc: Document): Element {
  const el = doc.querySelector('[data-scope="matrix-code"][data-part="root"]')
  if (!el)
    throw new Error('找不到 matrix-code 的 root 部件')
  return el
}
 ...
 * 两条分开与形状无关：码眼那条要单独承 `--xh-matrix-code-eye-fg`，并成一条那个变量就无处落地。
 */
...
export const matrixCodeSuite: ConformanceSuite = {
  component: 'matrix-code',
  anatomy: matrixCodeAnatomy,
  keyboard: matrixCodeKeyboard,
  ...
  cases: [
    ...
    {
      name: '不认识的码制：落 error 态、一个模块都不铺，data-format 原样写上给作者看',
      spec: { apg: APG },
      props: { format: 'ean13', value: URL_23 },
      initial: {
        counts: { root: 1 },
        parts: {
          root: {
      ...
      steps: [
        {
          kind: 'raw',
          why: '静默退回 qr 会画出一张按错码制扫出来的码；钉住的是"什么都不画"，只能数子元素',
          run: expectNothingPainted,
        },
        {
          kind: 'setProps',
      ...
      steps: [
        {
          ...
          run: ({ doc, adapterName }) => {
            ...
            const logo = doc.querySelector('[data-scope="matrix-code"][data-part="logo"]')!
