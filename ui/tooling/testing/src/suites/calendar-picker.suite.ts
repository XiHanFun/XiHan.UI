// 提供 calendar-picker.suite 相关实现。

import type { ConformanceSuite, FixtureNode } from '../conformance/types'
import { buildMonthGrid, buildWeekDays, calendarPickerAnatomy, calendarPickerKeyboard } from '@xihan-ui/headless'
...
function expectText(doc: Document, part: string, want: string, why: string): void {
  const got = doc.querySelector<HTMLElement>(`[data-scope="calendar-picker"][data-part="${part}"]`)?.textContent ?? ''
  if (got !== want)
...
export const calendarPickerSuite: ConformanceSuite = {
  component: 'calendar-picker',
  anatomy: calendarPickerAnatomy,
  keyboard: calendarPickerKeyboard,
  fixture: FIXTURE,
  cases: [
    {
      ...
      steps: [
        {
          ...
          run: ({ doc }) => {
            const triggers = [...doc.querySelectorAll<HTMLElement>('[data-scope="calendar-picker"][data-part="cell-trigger"]')]
    ...
    },
    {
      ...
      spec: { apg: APG },
      props: BASE_PROPS,
      covers: ['calendar-picker.kbd.tab'],
      steps: [
        singleTabStop('calendar-picker', 'cell-trigger', 'grid'),
    ...
    },
    {
      ...
      props: BASE_PROPS,
      covers: ['calendar-picker.kbd.prev-day', 'calendar-picker.kbd.next-day', 'calendar-picker.kbd.prev-week', 'calendar-picker.kbd.next-week'],
      steps: [
        { kind: 'focus', part: `cell-trigger[${at(ANCHOR)}]` },
        {
          kind: 'key',
    ...
    },
    {
      ...
      spec: { apg: `${APG}#kbd_label` },
      props: BASE_PROPS,
      covers: ['calendar-picker.kbd.week-start', 'calendar-picker.kbd.week-end'],
      steps: [
        { kind: 'focus', part: `cell-trigger[${at(ANCHOR)}]` },
        {
          kind: 'key',
    ...
    },
    {
      ...
      spec: { apg: `${APG}#kbd_label` },
      props: BASE_PROPS,
      covers: ['calendar-picker.kbd.prev-month', 'calendar-picker.kbd.next-month', 'calendar-picker.kbd.select'],
      steps: [
        { kind: 'focus', part: `cell-trigger[${at(ANCHOR)}]` },
        {
          kind: 'key',
    ...
    },
    {
      ...
      spec: { apg: `${APG}#kbd_label` },
      props: BASE_PROPS,
      covers: ['calendar-picker.kbd.prev-year', 'calendar-picker.kbd.next-year'],
      steps: [
        { kind: 'focus', part: `cell-trigger[${at(ANCHOR)}]` },
    ...
    },
    {
      ...
      spec: { apg: `${APG}#kbd_label` },
      props: BASE_PROPS,
      covers: ['calendar-picker.kbd.select'],
      steps: [
        { kind: 'focus', part: `cell-trigger[${at(ANCHOR)}]` },
        {
          kind: 'key',
    ...
    {
      // 翻月钮是原生按钮，Space 与 Enter 都是激活键；格子的确认键在 grid 的 keydown 里收口，keydown 那一刻就选中
      name: 'Space / Enter 按住与触屏按下：翻月钮与日期格各自投影 data-pressed，抬起、失焦或指针取消撤下；选中与按压互相独立',
      spec: { adr: 'press-channel' },
      covers: ['calendar-picker.kbd.press'],
      props: BASE_PROPS,
      steps: [
        heldPress('calendar-picker', 'prev-trigger'),
        heldPress('calendar-picker', 'next-trigger'),
        heldPress('calendar-picker', 'cell-trigger', { value: ANCHOR }),
        {
          kind: 'settle',
          until: { attr: { part: `cell-trigger[${at(ANCHOR)}]`, name: 'data-pressed', value: null } },
          expect: { parts: { ...selection(ANCHOR), 'prev-trigger': { 'data-pressed': null }, 'next-trigger': { 'data-pressed': null } } },
        },
      ],
    },
    {
      ...
      steps: [
        heldPress('calendar-picker', 'prev-year-trigger'),
        heldPress('calendar-picker', 'next-year-trigger'),
        heldPress('calendar-picker', 'heading-month-trigger'),
        heldPress('calendar-picker', 'heading-year-trigger'),
      ],
    },
    {
      ...
      steps: [
        heldPressIgnored('calendar-picker', 'cell-trigger', 'min 之前的日子不接受按压', { value: '2024-02-01' }),
        heldPressIgnored('calendar-picker', 'prev-trigger', '到界的上一月按不动'),
        heldPressIgnored('calendar-picker', 'next-trigger', '到界的下一月按不动'),
      ],
    },
    {
      ...
      steps: [
        heldPressIgnored('calendar-picker', 'cell-trigger', '只读时格子不接受按压', { value: ANCHOR }),
        heldPress('calendar-picker', 'next-trigger'),
        { kind: 'setProps', props: { readOnly: false, disabled: true } },
        heldPressIgnored('calendar-picker', 'cell-trigger', '整张禁用时格子不接受按压', { value: ANCHOR }),
        heldPressIgnored('calendar-picker', 'next-trigger', '整张禁用时翻月钮不接受按压'),
      ],
    },
    {
    ...
    },
    {
      ...
      props: BASE_PROPS,
      steps: [
        nativeActivation('calendar-picker', 'prev-trigger'),
        nativeActivation('calendar-picker', 'next-trigger'),
        {
          kind: 'click',
          part: 'next-trigger',
          expect: {
