// @xihan-ui/testing —— 跨适配器一致性套件运行时（私有，不发布）。

export { applyStep } from './conformance/apply-step'
export type { ApplyContext } from './conformance/apply-step'
export { attachHost } from './conformance/attach-host'
export { installCssAnimationMock } from './conformance/css-animation-mock'
export { pendingFrames, settleFrame, settleTeardown } from './conformance/frame'
export { checkExpectation } from './conformance/match'
export { recordTrace, runConformance, runParity } from './conformance/run'
export type { RunOptions } from './conformance/run'
export type {
  ActiveElementExpectation,
  ActiveElementRef,
  AdapterEvent,
  AdapterHarness,
  AdapterName,
  AttrExpectation,
  ConformanceCase,
  ConformanceSuite,
  DomSnapshot,
  Fixture,
  FixtureNode,
  KeyName,
  ModifierKey,
  PartRef,
  PartSnapshot,
  RawStepContext,
  SettleCondition,
  SnapshotExpectation,
  Step,
  StepWithExpect,
  TestHooks,
} from './conformance/types'
export { coveredRows, danglingCovers, missingKeyboardRows } from './machine/transition-coverage'
export { collectDomSnapshot } from './snapshot/collect'
export type { CollectOptions } from './snapshot/collect'
export { collectComputedSnapshot, COMPUTED_PROPS, formatComputedSnapshot, PLATFORM_PAINTED, runComputedSnapshot } from './snapshot/computed'
export type { ComputedCollectOptions, ComputedSnapshot, ComputedSnapshotOptions } from './snapshot/computed'
export { normalizeAttrs } from './snapshot/normalize'
export { accordionSuite } from './suites/accordion.suite'
export { affixSuite } from './suites/affix.suite'
export { alertSuite } from './suites/alert.suite'
export { allSuites } from './suites/all'
export { anchorSuite } from './suites/anchor.suite'
export { approvalSuite } from './suites/approval.suite'
export { avatarGroupSuite } from './suites/avatar-group.suite'
export { avatarSuite } from './suites/avatar.suite'
export { backTopSuite } from './suites/back-top.suite'
export { badgeSuite } from './suites/badge.suite'
export { barCodeSuite } from './suites/bar-code.suite'
export { breadcrumbSuite } from './suites/breadcrumb.suite'
export { buttonGroupSuite } from './suites/button-group.suite'
export { buttonSuite } from './suites/button.suite'
export { calendarPickerSuite } from './suites/calendar-picker.suite'
export { calendarRangePickerSuite } from './suites/calendar-range-picker.suite'
export { cardSuite } from './suites/card.suite'
export { carouselSuite } from './suites/carousel.suite'
export { cartesianChartSuite } from './suites/cartesian-chart.suite'
export { cascaderSuite } from './suites/cascader.suite'
export { checkboxGroupSuite } from './suites/checkbox-group.suite'
export { checkboxSuite } from './suites/checkbox.suite'
export { clipboardSuite } from './suites/clipboard.suite'
export { codeViewSuite } from './suites/code-view.suite'
export { collapsibleSuite } from './suites/collapsible.suite'
export { colorFieldSuite } from './suites/color-field.suite'
export { colorPickerSuite } from './suites/color-picker.suite'
export { colorSliderSuite } from './suites/color-slider.suite'
export { colorSwatchPickerSuite } from './suites/color-swatch-picker.suite'
export { colorSwatchSuite } from './suites/color-swatch.suite'
export { comboboxSuite } from './suites/combobox.suite'
export { commandSuite } from './suites/command.suite'
export { contextMenuSuite } from './suites/context-menu.suite'
export { dateFieldSuite } from './suites/date-field.suite'
export { datePickerSuite } from './suites/date-picker.suite'
export { dateRangePickerSuite } from './suites/date-range-picker.suite'
export { descriptionsSuite } from './suites/descriptions.suite'
export { dialogSuite } from './suites/dialog.suite'
export { diffViewSuite } from './suites/diff-view.suite'
export { downloadTriggerSuite } from './suites/download-trigger.suite'
export { drawerSuite } from './suites/drawer.suite'
export { editableSuite } from './suites/editable.suite'
export { emptyStateSuite } from './suites/empty-state.suite'
export { fieldArraySuite } from './suites/field-array.suite'
export { fieldSuite } from './suites/field.suite'
export { fieldsetSuite } from './suites/fieldset.suite'
export { fileUploadSuite } from './suites/file-upload.suite'
export { flexSuite } from './suites/flex.suite'
export { floatButtonSuite } from './suites/float-button.suite'
export { floatingPanelSuite } from './suites/floating-panel.suite'
export { formSuite } from './suites/form.suite'
export { gradientTextSuite } from './suites/gradient-text.suite'
export { gridSuite } from './suites/grid.suite'
export { heatmapSuite } from './suites/heatmap.suite'
export { highlightSuite } from './suites/highlight.suite'
export { hoverCardSuite } from './suites/hover-card.suite'
export { iconWrapperSuite } from './suites/icon-wrapper.suite'
export { iconSuite } from './suites/icon.suite'
export { imageCropperSuite } from './suites/image-cropper.suite'
export { IMAGE_VIEWER_CONTENT_CHILDREN, IMAGE_VIEWER_PENDING_ITEMS, imageViewerAtIndex, imageViewerProps, imageViewerSuite, openImageViewer } from './suites/image-viewer.suite'
export { imageSuite } from './suites/image.suite'
export { infiniteScrollSuite } from './suites/infinite-scroll.suite'
export { inputGroupSuite } from './suites/input-group.suite'
export { jsonViewerSuite } from './suites/json-viewer.suite'
export { kbdSuite } from './suites/kbd.suite'
export { layoutSuite } from './suites/layout.suite'
export { listSuite } from './suites/list.suite'
export { listboxSuite } from './suites/listbox.suite'
export { loadingBarSuite } from './suites/loading-bar.suite'
export { logSuite } from './suites/log.suite'
export { markdownStreamSuite } from './suites/markdown-stream.suite'
export { marqueeSuite } from './suites/marquee.suite'
export { masonrySuite } from './suites/masonry.suite'
export { matrixCodeSuite } from './suites/matrix-code.suite'
export { mentionSuite } from './suites/mention.suite'
export { menuSuite } from './suites/menu.suite'
export { menubarSuite } from './suites/menubar.suite'
export { messageFeedSuite } from './suites/message-feed.suite'
export { navigationMenuSuite } from './suites/navigation-menu.suite'
export { notificationSuite } from './suites/notification.suite'
export { numberAnimationSuite } from './suites/number-animation.suite'
export { numberFieldSuite } from './suites/number-field.suite'
export { pageHeaderSuite } from './suites/page-header.suite'
export { paginationSuite } from './suites/pagination.suite'
export { passwordInputSuite } from './suites/password-input.suite'
export { pieChartSuite } from './suites/pie-chart.suite'
export { pinInputSuite } from './suites/pin-input.suite'
export { popconfirmSuite } from './suites/popconfirm.suite'
export { popoverSuite } from './suites/popover.suite'
export { progressSuite } from './suites/progress.suite'
export { promptInputSuite } from './suites/prompt-input.suite'
export { questionFlowSuite } from './suites/question-flow.suite'
export { radioGroupSuite } from './suites/radio-group.suite'
export { ratingSuite } from './suites/rating.suite'
export { reasoningSuite } from './suites/reasoning.suite'
export { resizableSuite } from './suites/resizable.suite'
export { scrollAreaSuite } from './suites/scroll-area.suite'
export { scrollbarSuite } from './suites/scrollbar.suite'
export { segmentedSuite } from './suites/segmented.suite'
export { selectSuite } from './suites/select.suite'
export { separatorSuite } from './suites/separator.suite'
export { dispatchClickOnDisabled } from './suites/shared/disabled-press'
export { nativeActivation, singleTabStop } from './suites/shared/native-activation'
export { heldPress, heldPressIgnored } from './suites/shared/press-channel'
export { sideNavSuite } from './suites/side-nav.suite'
export { signaturePadSuite } from './suites/signature-pad.suite'
export { skeletonSuite } from './suites/skeleton.suite'
export { sliderSuite } from './suites/slider.suite'
export { sortableSuite } from './suites/sortable.suite'
export { spinnerSuite } from './suites/spinner.suite'
export { splitterSuite } from './suites/splitter.suite'
export { statisticSuite } from './suites/statistic.suite'
export { stepsSuite } from './suites/steps.suite'
export { switchSuite } from './suites/switch.suite'
export { tableSuite } from './suites/table.suite'
export { tabsSuite } from './suites/tabs.suite'
export { tagGroupSuite } from './suites/tag-group.suite'
export { tagSuite } from './suites/tag.suite'
export { tagsInputSuite } from './suites/tags-input.suite'
export { textFieldSuite } from './suites/text-field.suite'
export { timeFieldSuite } from './suites/time-field.suite'
export { timePickerSuite } from './suites/time-picker.suite'
export { timeRangePickerSuite } from './suites/time-range-picker.suite'
export { timelineSuite } from './suites/timeline.suite'
export { timerSuite } from './suites/timer.suite'
export { timestampSuite } from './suites/timestamp.suite'
export { toastSuite } from './suites/toast.suite'
export { toggleGroupSuite } from './suites/toggle-group.suite'
export { toggleSuite } from './suites/toggle.suite'
export { toolCallSuite } from './suites/tool-call.suite'
export { toolbarSuite } from './suites/toolbar.suite'
export { tooltipSuite } from './suites/tooltip.suite'
export { tourSuite } from './suites/tour.suite'

export { transferSuite } from './suites/transfer.suite'
export { treeSelectSuite } from './suites/tree-select.suite'
export { treeSuite } from './suites/tree.suite'
export { truncateSuite } from './suites/truncate.suite'
export { typographySuite } from './suites/typography.suite'
export { virtualizerSuite } from './suites/virtualizer.suite'
export { watermarkSuite } from './suites/watermark.suite'
