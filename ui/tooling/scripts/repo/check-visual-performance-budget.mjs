...
/** pnpm gate 的步骤写在模块清单里，不在 package.json 的 gate 串上。 */
const GATE_MANIFEST = 'tooling/scripts/gate.modules.mjs'
const { MODULES } = await import(pathToFileURL(join(process.cwd(), GATE_MANIFEST)).href)
const gateSteps = MODULES.flatMap(module => module.steps).join('\n')
...
requireText(gateSteps, 'check-visual-performance-budget.mjs', GATE_MANIFEST, '预算静态门禁没有接进 pnpm gate')
...
const expectedSizeSources = [
  ...
  ['.size-limit.css.json', 'node tooling/scripts/skin/check-skin-size.mjs'],
]
