#!/usr/bin/env node
// 门禁:tooling/scripts(连同各模块子目录)与 scripts 下的每个 check-*.mjs、gen-*.mjs 以及登记在
// WRITES_FILES 里的生成器,都必须接进 ui/package.json 的某个 script 或 gate 的模块清单。
//
...
// 检查与生成系统自己的完整性。
//
// gate 的步骤写在 tooling/scripts/gate.modules.mjs 里,由 tooling/scripts/gate.mjs 串行执行:
// 清单里的每一步按 `gate[<模块>]` 计作接线来源;模块目录与清单里的模块 id 必须一一对应。
import { existsSync } from 'node:fs'
import { readdir, readFile } from 'node:fs/promises'
import { join } from 'node:path'
import { pathToFileURL } from 'node:url'

const SCRIPTS_ROOT = 'tooling/scripts'
const ROOT_PKG = 'package.json'
const GATE_RUNNER = `${SCRIPTS_ROOT}/gate.mjs`
const GATE_MANIFEST = `${SCRIPTS_ROOT}/gate.modules.mjs`
/** tooling/scripts 下不是 gate 模块的子目录。 */
const NOT_MODULES = new Set(['lib'])

const { MODULES } = await import(pathToFileURL(join(process.cwd(), GATE_MANIFEST)).href)
const moduleDirs = (await readdir(SCRIPTS_ROOT, { withFileTypes: true }))
  .filter(entry => entry.isDirectory() && !NOT_MODULES.has(entry.name))
  .map(entry => entry.name)
const SCRIPT_DIRS = [SCRIPTS_ROOT, ...moduleDirs.map(dir => `${SCRIPTS_ROOT}/${dir}`), 'scripts']

/**
 * 这张门禁不看适配器，收尾行照原样打出来，免得读输出的人以为它顺带核过 React。
 *
 * SCRIPT_DIRS 是脚本所在的目录（tooling/scripts 连同各模块子目录、scripts），不是 vue / react / web-components——检查脚本与
...
const BASELINE_GUARDED = {
  'build-public-surface.mjs': 'tooling/scripts/package/check-public-surface.mjs',
...
/** package.json 的 script 与 gate 清单里的每一步，一并当作接线来源。 */
const entries = [
  ...Object.entries(root.scripts ?? {}),
  ...MODULES.flatMap(module => module.steps.map(step => [`gate[${module.id}]`, step])),
]
...
function isGuardingGate(name, body) {
  if (name !== 'gate' && !name.startsWith('gate:') && !name.startsWith('gate['))
    return false
...
// gate 走运行器;模块目录与清单一一对应,多出来的目录里的检查不会被任何模块执行
if (!normalize(root.scripts?.gate ?? '').includes(GATE_RUNNER))
  errors.push(`package.json 的 gate 没有调用 ${GATE_RUNNER}——模块清单 ${GATE_MANIFEST} 里的步骤不会执行`)
const manifestIds = new Set(MODULES.map(module => module.id))
for (const dir of moduleDirs) {
  if (!manifestIds.has(dir))
    errors.push(`${SCRIPTS_ROOT}/${dir}/ 不是 ${GATE_MANIFEST} 里的模块——新模块先在清单里登记,不是模块的目录登进本脚本的 NOT_MODULES`)
}
for (const id of manifestIds) {
  if (!moduleDirs.includes(id))
    errors.push(`${GATE_MANIFEST} 里的模块 ${id} 没有对应的 ${SCRIPTS_ROOT}/${id}/ 目录`)
}
/** 真的用来放行过的生成器。 */
...
for (const [path, name] of checks) {
  ...
  if (referrersOf(path).length === 0) {
    errors.push(referenceHint(path, name, '没接进任何 pnpm script,也不在 gate 清单里——写了却不跑,等于没写'))
  }
}
...
for (const [path, name] of generators) {
  ...
  const byGate = entries.some(([ref, body]) => normalize(body).includes(path) && isGuardingGate(ref, body))
...
console.log(
  `[check-wiring] 通过:${checks.size} 个检查脚本、${generators.size} 个生成器全部接入 script 或 gate 清单(${MODULES.length} 个模块)`
