...
try {
  ...
  console.error(`[check-motion-coverage] ✗ 读不到 ${TABLE}——先跑 node tooling/scripts/motion/check-motion-coverage.mjs --update 落基线`)
  process.exit(1)
}
...
for (const file of silent) {
  ...
  problems.push(
    ...
    + `或者跑 node tooling/scripts/motion/check-motion-coverage.mjs --update 把它登记进 ${TABLE} 并写清它为什么不需要`,
  )
}
