...
const BUSY_EXEMPT = {
  ...
  // 条子与卡片自己就是 role=status 的活区：报 busy 会让读屏把「正在上传」这句压到完事才念，
  // 而那句话正是加载态要告诉用户的；在途由可见文案与转圈承载，收尾时换语气再念一遍
  'toast': '本节点就是活区，busy 会压住它要念的那句「正在进行」',
  'notification': '本节点就是活区，busy 会压住它要念的那句「正在进行」',
}
...
const BUSY_WITHOUT_SKIN = {
  ...
  // 以下八家同 table：aria-busy 落在会被换掉的条目容器上，不是刚被按下的控件
  ...
  'select': '取数期那一格由只在取数期在场的 loading 部件承载（收起时 hidden），承载它的是部件的在场与否，不是一条按属性分档的规则',
  'transfer': '取数期那一格由只在取数期在场的 loading 部件承载（收起时 hidden），承载它的是部件的在场与否，不是一条按属性分档的规则',
  'tree': '取数期那一格由只在取数期在场的 loading 部件承载（收起时 hidden），承载它的是部件的在场与否，不是一条按属性分档的规则',
  'tree-select': '取数期那一格由只在取数期在场的 loading 部件承载（收起时 hidden），承载它的是部件的在场与否，不是一条按属性分档的规则',
}
