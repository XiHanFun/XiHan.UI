...
const truth = {
  ...
  gate里的结构检查数: {
    ...
    async value() {
      const { MODULES } = await import(pathToFileURL(join(uiRoot, 'tooling/scripts/gate.modules.mjs')).href)
      return new Set(MODULES.flatMap(module => module.steps.join(' ').match(/check-[a-z-]+\.mjs/g) ?? [])).size
    },
  },
  单独的gate脚本数: {
