...
125 components, each shipping a headless core, a Vue component, a custom element, and a default skin.
...
| `@xihan-ui/headless` | 125 components as anatomy + machine + `connect` — no styles, no framework |
...
pnpm build
pnpm size         # bundle size ratchet — builds, then checks the 33 budgets in .size-limit.json
```
