...
132 components, each shipping a headless core, Vue and React components, a custom element, and a default skin.
...
| `@xihan-ui/headless` | 132 components as anatomy + machine + `connect` — no styles, no framework |
...
pnpm build
pnpm size         # bundle size ratchet — builds, then checks the 37 budgets in .size-limit.json
```
