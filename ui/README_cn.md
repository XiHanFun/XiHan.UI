...
104 个组件，每个都有 headless 内核、Vue 组件、自定义元素与默认皮肤。
...
| `@xihan-ui/headless` | 104 个组件的 anatomy + machine + `connect`，无样式、无框架 |
...
pnpm build
pnpm size         # 体积棘轮：先构建，再核对 .size-limit.json 里的 25 条产物限额
```
