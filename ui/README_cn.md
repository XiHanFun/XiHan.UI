...
136 个组件，每个都有 headless 内核、Vue 与 React 组件、自定义元素和默认皮肤。
...
| `@xihan-ui/headless` | 136 个组件的 anatomy + machine + `connect`，无样式、无框架 |
...
pnpm build
pnpm size         # 体积棘轮：先构建，再核对 .size-limit.json 里的 40 条产物限额
```
