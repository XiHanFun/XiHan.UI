...
128 个组件，每个都有 headless 内核、Vue 与 React 组件及自定义元素；127 个视觉组件另有默认皮肤，renderless Hotkeys 不伪造视觉层。
...
| `@xihan-ui/headless` | 128 个组件的 anatomy + machine + `connect`，无样式、无框架 |
...
pnpm build
pnpm size         # 体积棘轮：先构建，再核对 .size-limit.json 里的 37 条产物限额
```
