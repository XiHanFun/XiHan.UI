...
**与 `scroll-area` 共用同一份几何。** 滚动量 ↔ 滑块几何的换算搬到 `shared/scroll-geometry`，两个组件都从那里取，滑块长度、能走的行程与 RTL 换算逐像素一致；`scroll-area` 原来的 `SCROLL_AREA_*` 导出名原样再导出，公开面不变。厚度与间隙也收进了令牌：新增 `--xh-scrollbar-thickness-sm/md/lg`、`--xh-scrollbar-gutter`、`--xh-scrollbar-min-thumb`，`scroll-area` 的皮肤改从这里取，两处放在一起长得一样。同时把一直没人用的 `--xh-bg-scrollbar-track` 接上——`scroll-area` 的轨道底色此前钉在 `--xh-bg-subtle` 上，换肤时不跟随；深色下这一处会比原来略深一档。

滚动容器要在滚动条挂载时就在 DOM 里：监听器只挂一次，晚到的容器接不上。查不到时投一条 `scrollbar.missing-scrollable` 诊断，不静默。容器里内容长短变了会自动重量（`MutationObserver` 盯着子树，一拍内合并成一次），量不到的场合另有 `api.measure()`。
