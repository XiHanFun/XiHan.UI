...
**`select` 多选标签行封顶：不给 `maxTagCount` 时最多摆 3 枚，其余合成一枚 `+N`；标签行与 `+N` 成为部件，三家适配器都渲出来、皮肤给了样子。**
...
- **新增部件 `overflow-tag`**（`api.getOverflowTagProps()`）：折起的标签合成的那一枚，带 `data-count`；没有折起的标签时带 `hidden`，不留空位。文字由 `api.overflowText` 给，走新增的 `translations.overflowTag(count)`，默认 `+N`。
- **三家适配器**：Vue 新增 `XhSelectTagList` / `XhSelectOverflowTag`，React 同名两件；`+N` 那一枚不写内容即显示 `overflowText`。Web Components 侧作者写 `<span data-xh-part="tag-list">` 与 `<span data-xh-part="overflow-tag">`，`+N` 由元素填字（留空归元素、写了内容归作者，与 `value-text` 同一条规矩）。根插槽 / 函数式 children 的载荷多一项 `overflowText`。
- **皮肤**：`tag-list` 是触发器里可压缩、裁溢出的一行（`flex: 0 1 auto; min-inline-size: 0; overflow: hidden`），行里的标签装不下时各自缩短带省略号，`+N` 不缩；`overflow-tag` 与标签同一副药丸身形，底色与字色压一档（`--xh-select-overflow-tag-bg` / `-fg` 回退 `--xh-bg-muted` / `--xh-fg-muted`）；新增覆盖槽 `--xh-select-tag-list-gap`。标签行露面时 `value-text` 让位（`display: none`），无选中时反过来——两者同时写在触发器里即可，不必再按 `tags.length` 二选一；`value-text` 留在 DOM 里，触发器的可及名仍从它取到完整的选中项文本。
...
- 解剖多两个部件（21 → 23）：`tag-list`、`overflow-tag`。按部件数断言过的用例要改。
