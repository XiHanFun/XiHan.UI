...
function typeMeta(id) {
  ...
  for (const sf of program.getSourceFiles()) {
    if (!typeFiles.includes(path.normalize(sf.fileName))) continue;
        ...
        for (const member of node.members) {
          ...
          if (key === "props" && ts.isTypeLiteralNode(member.type)) {
            for (const p of member.type.members) {
              if (!ts.isPropertySignature(p) || !p.type) continue;
              result.props.push({
          ...
          if (key === "state") result.states = unionMembers(member.type, sf);
          if (key === "event") result.events = unionMembers(member.type, sf);
          if (key === "guard") result.guards = unionMembers(member.type, sf);
        }
      }
...
function renderComponent(entry, category) {
  ...
  L.push("## 产物", "");
  L.push("| 层 | 值 |", "| --- | --- |");
  if (ad.tag) L.push(`| 自定义元素 | ${code(`<${ad.tag}>`)} |`);
  if (ad.components.length)
    L.push(`| Vue 组件 | ${ad.components.map(code).join(" ")} |`);
  if (ad.composable) L.push(`| 组合式函数 | ${code(ad.composable)} |`);
  L.push(
  ...
  if (tm.states || tm.events) {
    L.push("## 状态机", "");
    if (tm.states) L.push(`**状态**：${tm.states.map(code).join(" · ")}`, "");
    if (tm.events) L.push(`**事件**：${tm.events.map(code).join(" · ")}`, "");
    if (tm.guards) L.push(`**判据**：${tm.guards.map(code).join(" · ")}`, "");
  }
...
if (missing.length || extra.length) {
  ...
  if (extra.length) console.error(`manifest 登记了但代码里没有：${extra.join(", ")}`);
...
if (checkOnly) {
  ...
  for (const [file, content] of files) {
    ...
    if (current !== content) drifted.push(path.relative(repoRoot, file));
  }
  ...
  for (const f of fs.readdirSync(outDir)) {
    if (f.endsWith(".md") && !known.has(f)) fs.rmSync(path.join(outDir, f));
  }
